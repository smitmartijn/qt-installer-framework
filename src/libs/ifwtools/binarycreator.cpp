...
static void chmod755(const QString &absolutFilePath) {
  QFile::setPermissions(absolutFilePath,
...
T readInt(QIODevice *ioDevice, bool *ok, bool swap, bool peek = false) {
  const auto bytes =
      peek ? ioDevice->peek(sizeof(T)) : ioDevice->read(sizeof(T));
...
static QVersionNumber readMachOMinimumSystemVersion(QIODevice *device) {
  ...
  if (magic == FAT_MAGIC || magic == FAT_CIGAM || magic == FAT_MAGIC_64 ||
    ...
    for (uint32_t n = 0; n < nfat; ++n) {
      ...
      fat_arch.cpusubtype =
          static_cast<cpu_subtype_t>(readInt(device, &ok, swap));
      if (!ok)
        return QVersionNumber();

      fat_arch.offset = is64bit ? readInt<uint64_t>(device, &ok, swap)
                                : readInt(device, &ok, swap);
      if (!ok)
        return QVersionNumber();

      fat_arch.size = is64bit ? readInt<uint64_t>(device, &ok, swap)
                              : readInt(device, &ok, swap);
      if (!ok)
        return QVersionNumber();
  ...
  for (const auto &offset : machoOffsets) {
    ...
    for (uint32_t i = 0; i < header.ncmds; ++i) {
      ...
      case LC_VERSION_MIN_WATCHOS:
        ...
        versions.push_back(
            QVersionNumber(static_cast<int>(version >> 16),
      ...
      const qint64 remaining =
          static_cast<qint64>(cmdsize - sizeof(cmd) - sizeof(cmdsize));
...
static int assemble(Input input, const QInstaller::Settings &settings,
                    const BinaryCreatorArgs &args) {
#ifdef Q_OS_MACOS
  if (QInstaller::isInBundle(input.installerExePath)) {
    const QString bundle = input.installerExePath;
    // if the input file was a bundle
    const QSettings s(QString::fromLatin1("%1/Contents/Info.plist")
                          .arg(input.installerExePath),
                      QSettings::NativeFormat);
    input.installerExePath =
        QString::fromLatin1("%1/Contents/MacOS/%2")
            .arg(bundle)
            .arg(s.value(QLatin1String("CFBundleExecutable"),
                         QFileInfo(input.installerExePath).completeBaseName())
                     .toString());
  }

  const bool createDMG = input.outputPath.endsWith(QLatin1String(".dmg"));
  if (createDMG)
    input.outputPath.replace(input.outputPath.length() - 4, 4,
                             QLatin1String(".app"));
  ...
  if (isBundle) {
    ...
    if (file.open(QIODevice::ReadOnly))
      minimumSystemVersion =
          readMachOMinimumSystemVersion(&file).normalized().toString();

    const QString contentsResourcesPath =
        fi.filePath() + QLatin1String("/Contents/Resources/");
    ...
    const QString iconTargetFile =
        fi.completeBaseName() + QLatin1String(".icns");
    QFile::copy(iconFile, contentsResourcesPath + iconTargetFile);
    if (QDir(qApp->applicationDirPath() + QLatin1String("/qt_menu.nib"))
            .exists()) {
      copyDirectoryContents(
          qApp->applicationDirPath() + QLatin1String("/qt_menu.nib"),
    ...
    plistStream << QLatin1String("<?xml version=\"1.0\" encoding=\"UTF-8\"?>")
                << Qt::endl;
    ...
    plistStream << QLatin1String("\t<string>") << iconTargetFile
                << QLatin1String("</string>") << Qt::endl;
    plistStream << QLatin1String("\t<key>CFBundlePackageType</key>")
                << Qt::endl;
    plistStream << QLatin1String("\t<string>APPL</string>") << Qt::endl;
#define QUOTE_(x) #x
#define QUOTE(x) QUOTE_(x)
    plistStream << QLatin1String("\t<key>CFBundleShortVersionString</key>")
                << Qt::endl;
    plistStream << QLatin1String("\t<string>")
                << QLatin1String(QUOTE(IFW_VERSION_STR)) << ("</string>")
                << Qt::endl;
...
#undef QUOTE_
    ...
    plistStream << QLatin1String("\t<string>") << fi.completeBaseName()
                << QLatin1String("</string>") << Qt::endl;
    plistStream << QLatin1String("\t<key>CFBundleIdentifier</key>") << Qt::endl;
    plistStream << QLatin1String(
                       "\t<string>com.yourcompany.installerbase</string>")
                << Qt::endl;
    plistStream << QLatin1String("\t<key>NOTE</key>") << Qt::endl;
    plistStream << QLatin1String("\t<string>This file was generated by Qt "
                                 "Installer Framework.</string>")
                << Qt::endl;
    plistStream << QLatin1String("\t<key>NSPrincipalClass</key>") << Qt::endl;
    plistStream << QLatin1String("\t<string>NSApplication</string>")
                << Qt::endl;
    if (!minimumSystemVersion.isEmpty()) {
      plistStream << QLatin1String("\t<key>LSMinimumSystemVersion</key>")
                  << Qt::endl;
      plistStream << QLatin1String("\t<string>") << minimumSystemVersion
                  << QLatin1String("</string>") << Qt::endl;
    }
    plistStream << QLatin1String("</dict>") << Qt::endl;
    plistStream << QLatin1String("</plist>") << Qt::endl;

    input.outputPath = QString::fromLatin1("%1/Contents/MacOS/%2")
                           .arg(input.outputPath)
  ...
  if (!file.open()) {
    throw Error(
        QString::fromLatin1("Cannot copy %1 to %2: %3")
            .arg(input.installerExePath, input.outputPath, file.errorString()));
  }
  ...
  if (!instExe.copy(tempFile)) {
    throw Error(QString::fromLatin1("Cannot copy %1 to %2: %3")
                    .arg(instExe.fileName(), tempFile, instExe.errorString()));
  }

  QtPatch::patchBinaryFile(tempFile,
                           QByteArray("MY_InstallerCreateDateTime_MY"),
...
#if defined(Q_OS_WIN)
  // setting the windows icon must happen before we append our binary data -
  ...
  if (isBundle) {
    // no error handling as this is not fatal
    const QString copyscript =
        QDir::temp().absoluteFilePath(QLatin1String("copylibsintobundle.sh"));
  ...
  try {
    ...
    if (!exe.copy(input.outputPath)) {
      throw Error(
          QString::fromLatin1("Cannot copy %1 to %2: %3")
              .arg(exe.fileName(), input.outputPath, exe.errorString()));
    }
#else
    ...
    if (!args.createMaintenanceTool) {
      foreach (const QInstallerTools::PackageInfo &info, input.packages) {
        ...
        foreach (const QString &copiedFile, info.copiedFiles) {
          ...
          qDebug().nospace() << "Appending " << copiedFile << " ("
    ...
    qCritical("Error occurred while assembling the installer: %s",
  ...
  if (!out.rename(targetName)) {
    qCritical("Cannot write installer to %s: %s",
  ...
  if (isBundle && !args.signingIdentity.isEmpty()) {
    ...
    p.start(QLatin1String("codesign"),
            QStringList() << QLatin1String("--force") << QLatin1String("--deep")
    ...
    if (p.exitStatus() == QProcess::NormalExit) {
      if (p.exitCode() != 0) {
        qCritical("Failed to sign app bundle: running codesign failed "
                  "with exit code %d: %s",
                  p.exitCode(), p.readAllStandardError().constData());
        return EXIT_FAILURE;
      }
    }
  ...
  if (createDMG) {
    ...
    p.start(QLatin1String("/usr/bin/hdiutil"),
            QStringList() << QLatin1String("create") << imagePath
                          << QLatin1String("-srcfolder") << bundle
                          << QLatin1String("-ov") << QLatin1String("-volname")
                          << volumeName << QLatin1String("-fs")
                          << QLatin1String("HFS+"));
...
static int runRcc(const QStringList &args) {
  ...
  // Note: this does not run the rcc provided by Qt, this one is using the
  // compiled in binarycreator
  //       version. If it happens that resource mapping fails, we might need to
  //       adapt the code here...
...
static QSharedPointer<QInstaller::Resource>
createDefaultResourceFile(const QString &directory, const QString &binaryName) {
  QTemporaryFile projectFile(directory +
                             QLatin1String("/rccprojectXXXXXX.qrc"));
  if (!projectFile.open())
    throw Error(QString::fromLatin1(
        "Cannot create temporary file for generated rcc project file"));
  projectFile.close();

  const WorkingDirectoryChange wd(directory);
  const QString projectFileName =
      QFileInfo(projectFile.fileName()).absoluteFilePath();

  // 1. create the .qrc file
  if (runRcc(QStringList() << QLatin1String("rcc") << QLatin1String("-project")
                           << QLatin1String("-o")
  ...
  if (runRcc(QStringList() << QLatin1String("rcc") << QLatin1String("-binary")
                           << QLatin1String("-o") << binaryName
                           << projectFileName) != EXIT_SUCCESS) {
    throw Error(QString::fromLatin1("Cannot compile rcc project file."));
  }

  return QSharedPointer<QInstaller::Resource>(
      new QInstaller::Resource(binaryName, binaryName.toUtf8()));
}

static QList<QSharedPointer<QInstaller::Resource>>
createBinaryResourceFiles(const QStringList &resources) {
  ...
  foreach (const QString &resource, resources) {
    ...
    if (file.exists()) {
      ...
      const int status = runRcc(
          QStringList() << QLatin1String("rcc") << QLatin1String("-binary")
                        << QLatin1String("-o") << binaryName << fileName);
      if (status != EXIT_SUCCESS)
        continue;

      result.append(QSharedPointer<QInstaller::Resource>(
          new QInstaller::Resource(binaryName, binaryName.toUtf8())));
    }
  }
  return result;
...
void QInstallerTools::copyConfigData(const QString &configFile,
  ...
  QInstallerTools::copyWithException(sourceConfigFile, targetConfigFile,
                                     QLatin1String("configuration"));

  // Permissions might be set to bogus values
  QInstaller::setDefaultFilePermissions(targetConfigFile,
  ...
  const QString sourceConfigFilePath =
      QFileInfo(sourceConfigFile).absolutePath();
  for (int i = 0; i < children.count(); ++i) {
    ...
    qDebug().noquote() << QString::fromLatin1("Read dom element: <%1>%2</%1>.")
                              .arg(tagName, elementText);
      ...
      for (int j = 0; j < productImageNode.count(); ++j) {
        ...
        for (int k = 0; k < imageNode.count(); ++k) {
          ...
          const QString targetFile =
              targetDir + QLatin1Char('/') + productImageElement.text();
          const QFileInfo childFileInfo =
              QFileInfo(sourceConfigFilePath, productImageElement.text());
          QInstallerTools::copyWithException(childFileInfo.absoluteFilePath(),
                                             targetFile, imageChildName);
    ...
    domElement.replaceChild(dom.createTextNode(newName),
                            domElement.firstChild());
    QInstallerTools::copyWithException(elementFileInfo.absoluteFilePath(),
                                       targetFile, tagName);
...
void QInstallerTools::copyHighDPIImage(const QFileInfo &childFileInfo,
  ...
  const QFileInfo childFileInfoHighDpi =
      QFileInfo(childFileInfo.absolutePath(),
                childFileInfo.baseName() + scHighDpi + childFileInfo.suffix());
  if (childFileInfoHighDpi.exists()) {
    const QFileInfo tf(targetFile);
    const QString highDpiTarget = tf.absolutePath() + QLatin1Char('/') +
                                  tf.baseName() + scHighDpi + tf.suffix();
    QInstallerTools::copyWithException(childFileInfoHighDpi.absoluteFilePath(),
                                       highDpiTarget, childName);
  }
}
...
int QInstallerTools::createBinary(BinaryCreatorArgs args,
  ...
  foreach (const QString &packageDir, args.packagesDirectories) {
    if (!QFileInfo::exists(packageDir)) {
      argumentError = QString::fromLatin1(
  ...
  if (!args.filteredPackages.isEmpty() && args.onlineOnly) {
    argumentError = QString::fromLatin1(
        "Error: 'online-only' option cannot be used "
        "in conjunction with the 'include' or 'exclude' option. An "
        "'online-only' installer will never "
  ...
  if (!QFileInfo::exists(args.templateBinary)) {
    ...
    if (!QFileInfo::exists(args.templateBinary)) {
      argumentError = QString::fromLatin1(
          "Error: Template base binary not found at the specified location.");
      return EXIT_FAILURE;
    }
#else
    argumentError = QString::fromLatin1(
        "Error: Template not found at the specified location.");
    return EXIT_FAILURE;
#endif
  }
  ...
  if (!fi.exists()) {
    argumentError =
        QString::fromLatin1("Error: Config file %1 not found at the "
                            "specified location.")
  ...
  if (!fi.isFile()) {
    argumentError =
        QString::fromLatin1("Error: Configuration %1 is not a file.")
            .arg(fi.absoluteFilePath());
    return EXIT_FAILURE;
  }
  if (!fi.isReadable()) {
    argumentError =
        QString::fromLatin1("Error: Config file %1 is not readable.")
            .arg(fi.absoluteFilePath());
    return EXIT_FAILURE;
  }
  ...
  if (args.configFile.isEmpty()) {
    argumentError =
        QString::fromLatin1("Error: No configuration file selected.");
    return EXIT_FAILURE;
  }
  if (args.packagesDirectories.isEmpty() &&
      args.repositoryDirectories.isEmpty() && !args.compileResource &&
      !args.createMaintenanceTool) {
    argumentError = QString::fromLatin1(
        "Error: Both Package directory and Repository parameters missing.");
    return EXIT_FAILURE;
  }
  if (args.onlineOnly) {
    args.filteredPackages.append(
  ...
  try {
    const Settings settings = Settings::fromFileAndPrefix(
        args.configFile, QFileInfo(args.configFile).absolutePath());
    ...
    if (!args.repositoryDirectories.isEmpty()) {
      // 1.1; search packages
      PackageInfoVector precompressedPackages = createListOfRepositoryPackages(
          args.repositoryDirectories, &args.filteredPackages, args.ftype);
      ...
      foreach (const QString &dir, args.repositoryDirectories) {
        QDirIterator it(dir, QStringList(QLatin1String("*_meta.7z")),
    ...
    if (!args.packagesDirectories.isEmpty()) {
      // 2.1; search packages
      PackageInfoVector preparedPackages = createListOfPackages(
          args.packagesDirectories, &args.filteredPackages, args.ftype);
      // 2.2; copy the packages data and setup the packages vector with the
      // files we copied,
      //    must happen before copying meta data because files will be
      //    compressed if needed and meta data generation relies on this
    ...
    copyMetaData(tmpMetaDir, tmpRepoDir, packages, settings.applicationName(),
                 settings.version(), unite7zFiles);

    // 4; copy the configuration file and and icons etc.
    copyConfigData(args.configFile,
                   tmpMetaDir + QLatin1String("/installer-config"));
    {
      QSettings confInternal(tmpMetaDir +
                                 QLatin1String("/config/config-internal.ini"),
                             QSettings::IniFormat);
    ...
    if (!args.compileResource) {
      ...
      metaCollection.appendResource(
          createDefaultResourceFile(tmpMetaDir, generateTemporaryFileName()));
      ...
      createDefaultResourceFile(tmpMetaDir, QDir::currentPath() +
                                                QLatin1String("/update.rcc"));
  ...
  const ResourceCollection collection =
      input.manager.collectionByName("QResources");
  foreach (const QSharedPointer<QInstaller::Resource> &resource,
