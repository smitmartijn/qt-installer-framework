...
TempPathDeleter::TempPathDeleter(const QString &path) { m_paths.insert(path); }

TempPathDeleter::TempPathDeleter(const QStringList &paths)
    : m_paths(QSet<QString>(paths.begin(), paths.end())) {}

TempPathDeleter::~TempPathDeleter() { releaseAndDeleteAll(); }

QStringList TempPathDeleter::paths() const { return m_paths.values(); }

void TempPathDeleter::add(const QString &path) { m_paths.insert(path); }
...
/*!
    Returns the given \a size in a measuring unit suffixed human readable
   format, with \a precision marking the number of shown decimals.
*/
...
bool QInstaller::isLocalUrl(const QUrl &url) {
  return url.scheme().isEmpty() ||
         url.scheme().toLower() == QLatin1String("file");
}
...
void QInstaller::removeFiles(const QString &path, bool ignoreErrors) {
  ...
  foreach (const QFileInfo &fi, entries) {
    if (fi.isSymLink() || fi.isFile()) {
      ...
      QFile f(filePath);
      bool ok = f.remove();
      if (!ok) { // ReadOnly can prevent removing in Windows. Change permission
                 // and try again.
...
/*!
    ...
    @param ignoreErrors if @p true, errors will be silently ignored. Otherwise
   an exception will be thrown if removing fails.

    @throws QInstaller::Error if the directory cannot be removed and
   ignoreErrors is @p false
*/
void QInstaller::removeDirectory(const QString &path, bool ignoreErrors) {
  if (path.isEmpty()) // QDir("") points to the working directory! We never want
                      // to remove that one.
    return;

  QStringList dirs;
  QDirIterator it(
      path, QDir::NoDotAndDotDot | QDir::Dirs | QDir::NoSymLinks | QDir::Hidden,
...
public:
  explicit RemoveDirectoryThread(const QString &path, bool ignoreErrors = false,
                                 QObject *parent = nullptr)
      : QThread(parent), p(path), ignore(ignoreErrors) {
...
/*!
    \internal
*/
void QInstaller::removeDirectoryThreaded(const QString &path,
                                         bool ignoreErrors) {
  RemoveDirectoryThread thread(path, ignoreErrors);
  QEventLoop loop;
  QObject::connect(&thread, &RemoveDirectoryThread::finished, &loop,
                   &QEventLoop::quit);
  thread.start();
  loop.exec();
...
/*!
    Removes system generated files from \a path on Windows and macOS. Does
   nothing on Linux.
*/
...
/*!
    Sets permissions of file or directory specified by \a fileName to \c 644 or
   \c 755 based by the value of \a permissions.

    Returns \c true on success, \c false otherwise.
*/
bool QInstaller::setDefaultFilePermissions(const QString &fileName,
                                           DefaultFilePermissions permissions) {
  QFile file(fileName);
  return setDefaultFilePermissions(&file, permissions);
}

/*!
    Sets permissions of file or directory specified by \a file to \c 644 or \c
   755 based by the value of \a permissions. This is effective only on Unix
   platforms as \c setPermissions() does not manipulate ACLs. On Windows NTFS
   volumes this only unsets the legacy read-only flag regardless of the value of
   \a permissions.

    Returns \c true on success, \c false otherwise.
*/
bool QInstaller::setDefaultFilePermissions(QFile *file,
                                           DefaultFilePermissions permissions) {
  if (!file->exists()) {
    qCWarning(QInstaller::lcInstallerInstallLog)
        << "Target" << file->fileName() << "does not exists.";
    return false;
  }
  ...
  if (!file->setPermissions(
          static_cast<QFileDevice::Permission>(permissions))) {
    qCWarning(QInstaller::lcInstallerInstallLog)
        << "Cannot set default permissions for target" << file->fileName()
        << ":" << file->errorString();
    return false;
  }
  return true;
...
/*!
    \internal
*/
void QInstaller::copyDirectoryContents(const QString &sourceDir,
      ...
      if (!f.copy(target)) {
        throw Error(
            QCoreApplication::translate(
                "QInstaller", "Cannot copy file from \"%1\" to \"%2\": %3")
                .arg(QDir::toNativeSeparators(f.fileName()),
                     QDir::toNativeSeparators(target), f.errorString()));
      }
    }
  }
}
...
/*!
    \internal
*/
void QInstaller::moveDirectoryContents(const QString &sourceDir,
                                       const QString &targetDir) {
  Q_ASSERT(QFileInfo(sourceDir).isDir());
  Q_ASSERT(!QFileInfo::exists(targetDir) || QFileInfo(targetDir).isDir());
  if (!QDir().mkpath(targetDir)) {
  ...
  while (it.hasNext()) {
    const QFileInfo i(it.next());
    if (i.isDir()) {
      // only copy directories that are not the target to avoid loop dir
      // creations
      QString newSource = QDir(sourceDir).absoluteFilePath(i.fileName());
      if (QDir(newSource) != QDir(targetDir)) {
        moveDirectoryContents(newSource,
                              QDir(targetDir).absoluteFilePath(i.fileName()));
      }
    } else {
      QFile f(i.filePath());
      ...
      if (!f.rename(target)) {
        throw Error(
            QCoreApplication::translate(
                "QInstaller", "Cannot move file from \"%1\" to \"%2\": %3")
                .arg(QDir::toNativeSeparators(f.fileName()),
                     QDir::toNativeSeparators(target), f.errorString()));
      }
    }
...
/*!
    ...
    Creates directory \a path including all parent directories. Return \c true
   on success, \c false otherwise.

    On Windows \c QDir::mkpath() doesn't check if the leading directories were
   created elsewhere (i.e. in another thread) after the initial check that the
   given path requires creating also parent directories, and returns \c false.

    On Unix platforms this case is handled different by QFileSystemEngine
   though, which checks for \c EEXIST error code in case any of the recursive
   directories could not be created.

    Compared to \c QInstaller::mkpath() and \c QDir::mkpath() this function
   checks if each parent directory to-be-created were created elsewhere.
*/
...
QString QInstaller::generateTemporaryFileName(const QString &templ) {
  if (templ.isEmpty()) {
    ...
    if (!f.open()) {
      throw Error(QCoreApplication::translate("QInstaller",
                                              "Cannot open temporary file: %1")
                      .arg(f.errorString()));
    }
    return f.fileName();
  }

  static const QString characters = QLatin1String(
      "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890");
  QString suffix;
  for (int i = 0; i < 5; ++i)
    suffix += characters[QRandomGenerator::global()->generate() %
                         characters.length()];
  ...
  if (!f.open(QIODevice::WriteOnly)) {
    throw Error(
        QCoreApplication::translate(
            "QInstaller", "Cannot open temporary file for template %1: %2")
            .arg(templ, f.errorString()));
  }
  f.remove();
  return f.fileName();
}
...
QString QInstaller::getShortPathName(const QString &name) {
  ...
  const QString rc = QString::fromUtf16(
      reinterpret_cast<const ushort *>(buffer.data()), length - 1);
  return rc;
}
...
/*!
    ...
    Makes sure that capitalization of directory specified by \a name is
   canonical.
*/
...
*/
void QInstaller::setApplicationIcon(const QString &application,
                                    const QString &icon) {
  QFile iconFile(icon);
  if (!iconFile.open(QIODevice::ReadOnly)) {
    qCWarning(QInstaller::lcInstallerInstallLog)
        << "Cannot use" << icon
        << "as an application icon:" << iconFile.errorString();
    return;
  }
  ...
  DWORD newSize =
      sizeof(GRPICONDIR) + sizeof(GRPICONDIRENTRY) * (ig->idCount - 1);
  ...
  HANDLE updateRes = BeginUpdateResourceW(
      (wchar_t *)QDir::toNativeSeparators(application).utf16(), false);
  ...
  UpdateResourceW(updateRes, RT_GROUP_ICON, L"IDI_ICON1",
                  MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), newDir, newSize);
...
static quint64 symlinkSizeWin(const QString &path) {
  ...
  if (GetFileAttributesEx((wchar_t *)(path.utf16()), GetFileExInfoStandard,
...
/*!
    ...
    absolute path for the bundle can be retrieved with \a bundlePath. Works only
   on macOS.
*/
...
/*!
    Replaces the path \a before with the path \a after at the beginning of \a
   path and returns the replaced path. If \a before cannot be found in \a path,
   the original value is returned. If \a cleanPath is \c true, path is returned
   with directory separators normalized (that is, platform-native separators
   converted to "/") and redundant ones removed, and "."s and ".."s resolved (as
   far as possible). If \a cleanPath is \c false, path is returned as such.
   Default value is \c true.
*/
QString QInstaller::replacePath(const QString &path, const QString &before,
                                const QString &after, bool cleanPath) {
...
/*!
    Replaces \a imagePath with high dpi image. If high dpi image is not provided
   or high dpi screen is not in use, the original value is returned.
*/
void QInstaller::replaceHighDpiImage(QString &imagePath) {
  if (QGuiApplication::primaryScreen()->devicePixelRatio() >= 2) {
    QFileInfo fi(imagePath);
    QString highdpiPixmap = fi.absolutePath() + QLatin1Char('/') +
                            fi.baseName() + scHighDpi + fi.suffix();
...
/*!
    Copies an internal configuration file from \a source to \a target. The XML
   elements, and their children, specified by \a elementsToRemoveTags will be
   removed from the \a target file. All relative filenames referenced in the \a
   source configuration file will be also copied to the location of the \a
   target file.

    Throws \c QInstaller::Error in case of failure.
*/
void QInstaller::trimmedCopyConfigData(
    const QString &source, const QString &target,
    const QStringList &elementsToRemoveTags) {
  qCDebug(QInstaller::lcDeveloperBuild)
      << "Copying configuration file and associated data.";
  ...
  if (!xmlFile.copy(target)) {
    throw Error(QCoreApplication::translate(
                    "QInstaller", "Cannot copy file \"%1\" to \"%2\": %3")
                    .arg(source, target, xmlFile.errorString()));
  }
  ...
  foreach (auto elementTag, elementsToRemoveTags) {
    ...
    for (int i = 0; i < elementsToRemove.length(); i++) {
      ...
      qCDebug(QInstaller::lcDeveloperBuild)
          << "Removed dom element from target file:"
          << elementToRemove.toElement().text();
    }
  }

  const QDomNodeList children = dom.documentElement().childNodes();
  copyConfigChildElements(dom, children, QFileInfo(source).absolutePath(),
  ...
  qCDebug(QInstaller::lcDeveloperBuild)
      << "Finished copying configuration data.";
}

/*!
    \internal

    Recursively iterates over a list of QDomNode \a objects belonging to \a dom
   and their children accordingly, searching for relative file names. Found
   files are copied from \a sourceDir to \a targetDir.

    Throws \c QInstaller::Error in case of failure.
*/
void QInstaller::copyConfigChildElements(QDomDocument &dom,
                                         const QDomNodeList &objects,
  ...
  for (int i = 0; i < objects.length(); i++) {
    ...
    QInstaller::copyConfigChildElements(dom, elementChildren, sourceDir,
                                        targetDir);
    ...
    const QString newName =
        domElement.text().replace(regex, QLatin1String("_"));
    ...
    if (!QFile::copy(elementFileInfo.absoluteFilePath(), targetFile)) {
      throw Error(QCoreApplication::translate(
                      "QInstaller", "Cannot copy file \"%1\" to \"%2\".")
                      .arg(elementFileInfo.absoluteFilePath(), targetFile));
    }
  }
}
