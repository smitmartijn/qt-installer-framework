...
/*!
    ...
    \brief The OperationBlob class is a textual representation of an operation
   that can be instantiated and executed by the Qt Installer Framework.
*/

/*!
    \fn QInstaller::OperationBlob::OperationBlob(const QString &n, const QString
   &x)

    Constructs the operation blob with the given arguments, while \a n stands
   for the name part and \a x for the XML representation of the operation.
*/
...
/*!
    ...
    \inmodule QtInstallerFramework
    \brief The Resource class is an interface for wrapping a file as read only
   device.

    Resource is an interface for reading inside a file, but is not supposed to
   write to the file it wraps. The \c Resource class is created by passing a
   path to an existing binary (such as a zipped archive or a Qt resource file).

    The resource name can be set at any time using setName() or during
   construction. The segment supplied during construction represents the offset
   and size of the resource inside the file.
*/
...
Resource::Resource(const QString &path)
    : m_file(path), m_name(QFileInfo(path).fileName().toUtf8()),
...
/*!
    Destroys the resource. Calls close() if necessary before destroying the
   resource.
*/
Resource::~Resource() {
  if (isOpen())
...
/*!
    \reimp
 */
bool Resource::seek(qint64 pos) { return QIODevice::seek(pos); }

/*!
    Returns the name of the resource.
*/
QByteArray Resource::name() const { return m_name; }

/*!
    Sets the name of the resource to \a name.
*/
void Resource::setName(const QByteArray &name) { m_name = name; }
...
bool Resource::open() {
  ...
  if (!QIODevice::open(QIODevice::ReadOnly)) {
    setErrorString(tr("Cannot open resource %1 for reading.")
                       .arg(QString::fromUtf8(m_name)));
    return false;
  }
  return true;
}
...
/*!
    \reimp
 */
qint64 Resource::size() const { return m_segment.length(); }
...
/*!
    ...
    Copies the resource data of \a resource to a file called \a out. Throws
   Error on failure.
*/
  ...
  while (left > 0) {
    ...
    if (bytesRead != len) {
      throw QInstaller::Error(tr("Read failed after %1 bytes: %2")
                                  .arg(QString::number(resource->size() - left),
...
/*!
    ...
    \inmodule QtInstallerFramework
    \brief The ResourceCollection class is an abstraction that groups together a
   number of resources.

    The resources are supposed to be sequential, so the collection keeps them
   ordered once a new resource is added. The name can be set at any time using
   setName().
*/

/*!
    The class constructor creates an empty resource collection. By default the
   collection gets a unique name assigned using QUuid.
*/
ResourceCollection::ResourceCollection()
    : m_name(QUuid::createUuid().toByteArray()) {}

/*!
    The class constructor creates an empty resource collection with a name set
   to \a name.
*/
ResourceCollection::ResourceCollection(const QByteArray &name) : m_name(name) {}

/*!
    Returns the name of the resource collection.
*/
QByteArray ResourceCollection::name() const { return m_name; }

/*!
    Sets the name of the resource collection to \a name.
*/
void ResourceCollection::setName(const QByteArray &name) { m_name = name; }

/*!
    Appends \a resource to this collection. The collection takes ownership of \a
   resource.
 */
void ResourceCollection::appendResource(
...
/*!
    Appends a list of \a resources to this collection. The collection takes
   ownership of \a resources.
*/
void ResourceCollection::appendResources(
    const QList<QSharedPointer<Resource>> &resources) {
...
ResourceCollection::resourceByName(const QByteArray &name) const {
...
/*!
    ...
    \inmodule QtInstallerFramework
    \brief The ResourceCollectionManager class is an abstraction that groups
   together a number of resource collections.

    The resource collections it groups can be written to and read from a
   QFileDevice.
*/

/*!
    Reads the resource collection from the file \a dev. The \a offset argument
   is used to set the collection's resources segment information.
*/
void ResourceCollectionManager::read(QFileDevice *dev, qint64 offset) {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    const Range<qint64> segment =
        QInstaller::retrieveInt64Range(dev).moved(offset);
...
/*!
    Writes the resource collection to the file \a out. The \a offset argument is
   used to set the collection's segment information.
*/
Range<qint64> ResourceCollectionManager::write(QFileDevice *out,
                                               qint64 offset) const {
  ...
  foreach (const ResourceCollection &collection, m_collections) {
    ...
    foreach (const QSharedPointer<Resource> &resource, collection.resources()) {
      start += (sizeof(qint64)) // the number of bytes that get written and the
               + resource->name()
                     .size() // resource name (see QInstaller::appendByteArray)
               + (2 * sizeof(qint64)); // the resource range (see
    ...
    foreach (const QSharedPointer<Resource> &resource, collection.resources()) {
      ...
      QInstaller::appendInt64Range(
          out, Range<qint64>::fromStartAndLength(
                   start,
                   resource->size())); // the actual range once the table has
                                       // been written
    ...
    foreach (const QSharedPointer<Resource> &resource, collection.resources()) {
      if (!resource->open()) {
        throw QInstaller::Error(tr("Cannot open resource %1: %2")
                                    .arg(QString::fromUtf8(resource->name()),
                                         resource->errorString()));
      }
    ...
    table.insert(
        collection.name(),
        Range<qint64>::fromStartAndEnd(dataBegin, out->pos()).moved(offset));
  }
...
*/
ResourceCollection
ResourceCollectionManager::collectionByName(const QByteArray &name) const {
...
*/
void ResourceCollectionManager::insertCollection(
...
*/
void ResourceCollectionManager::clear() { m_collections.clear(); }
