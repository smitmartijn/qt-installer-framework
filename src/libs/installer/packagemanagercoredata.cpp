...
PackageManagerCoreData::PackageManagerCoreData(
    const QHash<QString, QString> &variables, const bool isInstaller) {
  // Add user defined variables before dynamic as user settings can affect
  // dynamic variables.
  setUserDefinedVariables(variables);
  addDynamicPredefinedVariables();
  // Set some common variables that may used e.g. as placeholder in some of the
  // settings variables or in a script or...
  addNewVariable(QLatin1String("InstallerDirPath"),
                 QCoreApplication::applicationDirPath());
  addNewVariable(QLatin1String("InstallerFilePath"),
  ...
  m_settings = Settings::fromFileAndPrefix(
      m_settingsFilePath, QFileInfo(m_settingsFilePath).absolutePath(),
  ...
  addNewVariable(QLatin1String("ProductVersion"),
                 replaceVariables(m_settings.version()));
  ...
  if (isInstaller) {
    ...
    addNewVariable(scTargetConfigurationFile,
                   m_settings.configurationFileName());
    addNewVariable(scStartMenuDir, m_settings.startMenuDir());
  } else {
#ifdef Q_OS_MACOS
    addNewVariable(scTargetDir,
  ...
  addNewVariable(scRemoveTargetDir,
                 replaceVariables(m_settings.removeTargetDir()));
}
...
/*!
    Set some common variables that may be used e.g. as placeholder in some of
   the settings variables or in a script or...
*/
void PackageManagerCoreData::addDynamicPredefinedVariables() {
...
#elif defined(Q_OS_MACOS)
  dir = QStandardPaths::standardLocations(QStandardPaths::ApplicationsLocation)
            .value(1);
  if (dir.isEmpty())
    dir =
  ...
  if (onWow64Or64bit == TRUE) {
    programfilesX86 =
        current.value(QLatin1String("ProgramFilesDir (x86)"), QString())
            .toString();
    programfilesX64 =
        current.value(QLatin1String("ProgramW6432Dir"), QString()).toString();
  } else {
    programfilesX86 =
        current.value(QLatin1String("ProgramFilesDir"), QString()).toString();
...
#ifdef Q_OS_WIN
  QSettingsWrapper user(
      QLatin1String("HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\"
                    "CurrentVersion\\Explorer\\User Shell Folders"),
      QSettings::NativeFormat);
  QSettingsWrapper system(
      QLatin1String("HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Windows\\"
                    "CurrentVersion\\Explorer\\Shell Folders"),
      QSettings::NativeFormat);

  const QString programs =
      user.value(QLatin1String("Programs"), QString()).toString();
  const QString allPrograms =
      system.value(QLatin1String("Common Programs"), QString()).toString();
  ...
  addNewVariable(QLatin1String("DesktopDir"),
                 replaceWindowsEnvironmentVariables(desktop));
...
#define QUOTE(x) QUOTE_(x)
  addNewVariable(QLatin1String("FrameworkVersion"),
                 QLatin1String(QUOTE(IFW_VERSION_STR)));
  // Undocumented, left for compatibility with scripts using the old key
  addNewVariable(QLatin1String("IFW_VERSION_STR"),
                 QLatin1String(QUOTE(IFW_VERSION_STR)));
#undef QUOTE
#undef QUOTE_
}

void PackageManagerCoreData::setUserDefinedVariables(
...
void PackageManagerCoreData::addNewVariable(const QString &key,
                                            const QString &value) {
  if (!m_variables.contains(key))
    m_variables.insert(key, value);
}

Settings &PackageManagerCoreData::settings() const { return m_settings; }

QStringList PackageManagerCoreData::keys() const { return m_variables.keys(); }

bool PackageManagerCoreData::contains(const QString &key) const {
  return m_variables.contains(key) || m_settings.containsValue(key);
}

bool PackageManagerCoreData::setValue(const QString &key,
...
QByteArray
PackageManagerCoreData::replaceVariables(const QByteArray &ba) const {
  ...
  while (true) {
    ...
    const QString name =
        QString::fromLocal8Bit(ba.mid(pos1 + 1, pos2 - pos1 - 1));
