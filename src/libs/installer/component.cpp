...
static const char *scClearCacheHint =
    QT_TR_NOOP("Clearing the cache directory and restarting the application "
               "may solve this.");
...
/*!
    ...
    \brief The SortingPriorityLessThan class sets an increasing sorting order
   for child components.

    If the component contains several children and has this sorting priority
   set, the child list is sorted so that the child component with the lowest
   priority is placed on top.
*/

/*!
    \fn QInstaller::Component::SortingPriorityLessThan::operator() (const
...
/*!
    ...
    \brief The SortingPriorityGreaterThan class sets a decreasing sorting
   priority for child components.

    If the component contains several children and has this sorting priority
   set, the child list is sorted so that the child component with the highest
   priority is placed on top.
*/

/*!
    \fn QInstaller::Component::SortingPriorityGreaterThan::operator() (const
   Component *lhs, const Component *rhs) const
...
/*!
    ...
    \brief The components this component depends on. This is a read-only
   property.
*/
...
/*!
    ...
    \brief The value of the \c <AutoDependOn> element in the package information
   file.
*/
...
/*!
    ...
    If this component is not downloaded from an online repository, returns an
   empty QUrl.
*/
...
/*!
    ...
    \brief Whether the component is currently enabled. The property is both
...
/*!
    ...
    \brief The tree name of the component. Specifies the location of the
   component in the install tree view.

    \note If the tree name is not set, the tree view is organized based on the
   component name.

    \note Must be unique, the tree name must not conflict with other component's
   names or tree names.
*/
...
/*!
    \fn QInstaller::Component::valueChanged(const QString &key, const QString
   &value)

    Emitted when the value of the variable with the name \a key changes to \a
   value.
...
Component::Component(PackageManagerCore *core)
  ...
  qRegisterMetaType<QList<QInstaller::Component *>>(
...
/*!
    Sets variables according to the values set in the package.xml file of a
   local \a package.
*/
...
/*!
    Sets variables according to the values set in the package.xml file of \a
   package. Also loads UI files, licenses and translations if they are
   referenced in the package.xml. If the \c PackageManagerCore object of this
   component is run as package viewer, then only sets the variables without
...
void Component::loadDataFromPackage(const Package &package) {
  ...
  setValue(scDownloadableArchives,
  ...
  setValue(scRequiresAdminRights,
           package.data(scRequiresAdminRights).toString());
  ...
  QString forced =
      package.data(scForcedInstallation, scFalse).toString().toLower();
  ...
  setValue(scCheckSha1CheckSum,
           package.data(scCheckSha1CheckSum, scTrue).toString().toLower());

  const auto treeNamePair =
      package.data(scTreeName).value<QPair<QString, bool>>();
  ...
  const QStringList uiList = QInstaller::splitStringWithComma(
      package.data(scUserInterfaces).toString());
  if (!uiList.isEmpty())
    loadUserInterfaces(QDir(scTwoArgs.arg(localTempPath(), name())), uiList);

#ifndef IFW_DISABLE_TRANSLATIONS
  const QStringList qms =
      QInstaller::splitStringWithComma(package.data(scTranslations).toString());
  if (!qms.isEmpty())
  ...
  if (operationsVariant.canConvert<QList<QPair<QString, QVariant>>>())
    m_operationsList =
        operationsVariant.value<QList<QPair<QString, QVariant>>>();
}
...
quint64 Component::updateUncompressedSize() {
  ...
  const bool installOrKeepInstalled =
      (installAction() == ComponentModelHelper::Install ||
...
*/
void Component::markAsPerformedInstallation() { d->m_newlyInstalled = true; }

/*!
    Returns a key and value based hash of all variables set for this component.
*/
QHash<QString, QString> Component::variables() const { return d->m_vars; }

/*!
    Returns the value of variable name \a key. If \a key is not known yet, \a
   defaultValue is returned.

    \note If a component is virtual and you ask for the component value with the
   key \c Default, it will always return \c false.
*/
...
/*!
    Removes all the values that have the \a key from the variables set for this
   component. Returns the number of values removed which is 1 if the key exists
   in the variables, and 0 otherwise.
*/
int Component::removeValue(const QString &key) { return d->m_vars.remove(key); }
...
void Component::setValue(const QString &key, const QString &value) {
  ...
  if (key ==
      scCheckable) // Non-checkable components can still be toggled in updater
  ...
  if (key == scForcedInstallation) {
    if (value == scTrue && !d->m_core->isUpdater() &&
        !PackageManagerCore::noForceInstallation()) {
  ...
  if (key == scAutoDependOn)
    packageManagerCore()->createAutoDependencyHash(name(), d->m_vars[key],
                                                   normalizedValue);
...
*/
PackageManagerCore *Component::packageManagerCore() const { return d->m_core; }

/*!
    Returns the parent of this component. For example, the parent of a component
   called \c org.qt-project.sdk.qt is \c org.qt-project.sdk if it exists.
*/
Component *Component::parentComponent() const { return d->m_parentComponent; }

/*!
    Appends \a component as a child of this component. If \a component already
   has a parent, it is removed from the previous parent. If the \a component
   contains several children and has the SortingPriorityGreaterThan() sorting
   priority set, the child list is sorted so that the child component with the
   highest priority is placed on top.
*/
void Component::appendComponent(Component *component) {
  ...
  if (!component->isVirtual()) {
    const QList<Component *> virtualChildComponents =
        d->m_allChildComponents.mid(d->m_childComponents.count());
    d->m_childComponents.append(component);
    std::sort(d->m_childComponents.begin(), d->m_childComponents.end(),
              SortingPriorityGreaterThan());
...
/*!
    Removes \a component if it is a child of this component. The component
   object still exists after the function returns. It is up to the caller to
   delete the passed \a component.
*/
...
/*!
    Returns a list of child components, including all descendants of the
   component's children.

    \note The returned list does include all children; non-virtual components as
   well as virtual components.
*/
...
*/
QString Component::name() const { return d->m_componentName; }
...
/*!
    ...
    to improve performance if the amount of components is huge and there are no
   script functions that need to be called before the installation starts.
*/
void Component::loadComponentScript(const bool postLoad) {
  const QString installScript(
      !postLoad ? d->m_scriptHash.value(scInstallScript).toString()
                : d->m_scriptHash.value(scPostLoadScript).toString());

  if (!localTempPath().isEmpty() && !installScript.isEmpty()) {
    evaluateComponentScript(
        scThreeArgs.arg(localTempPath(), name(), installScript), postLoad);
  }
}
...
/*!
    \internal
*/
void Component::evaluateComponentScript(const QString &fileName,
                                        const bool postScriptContent) {
  // introduce the component object as javascript value and call the name to
  // check that it was successful
  try {
    if (postScriptContent) {
      d->m_postScriptContext = d->scriptEngine()->loadInContext(
          scComponent, fileName, scComponentScriptTest.arg(name()));
    } else {
      d->m_scriptContext = d->scriptEngine()->loadInContext(
          scComponent, fileName, scComponentScriptTest.arg(name()));
    }
    ...
    // evaluateComponentScript is called with postScriptContent after we have
    // selected components and are about to install. Do not allow install if
    // unstable components are allowed as we then end up installing a component
    // which has invalid script.
...
*/
void Component::languageChanged() { callScriptMethod(scRetranslateUi); }

/*!
    Loads the translations matching the name filters \a qms inside \a directory.
   Only translations with a base name matching the current locale's name are
   loaded. For more information, see \l{Translating Pages}.
*/
void Component::loadTranslations(const QDir &directory,
                                 const QStringList &qms) {
  ...
  while (it.hasNext()) {
    ...
    if (!translations.isEmpty()) {
      ...
      if (!found) // don't load the file if it does match the UI language but is
                  // not allowed to be used
        continue;
    } else if (!uiLanguage.startsWith(QFileInfo(filename).baseName(),
                                      Qt::CaseInsensitive)) {
    ...
    if (translator->load(filename)) {
      // Do not throw if translator returns false as it may just be an
      // intentionally empty file. See also QTBUG-31031
...
/*!
    Loads the user interface files matching the name filters \a uis inside \a
   directory. The loaded interface can be accessed via userInterfaces() by using
   the class name set in the UI file.
*/
void Component::loadUserInterfaces(const QDir &directory,
                                   const QStringList &uis) {
  ...
  while (it.hasNext()) {
    ...
    if (!file.open(QIODevice::ReadOnly)) {
      throw Error(
          tr("Cannot open the requested UI file \"%1\": %2.\n\n%3")
              .arg(it.fileName(), file.errorString(), tr(scClearCacheHint)));
    }
    ...
    if (!widget) {
      throw Error(
          tr("Cannot load the requested UI file \"%1\": %2.\n\n%3")
              .arg(it.fileName(), loader.errorString(), tr(scClearCacheHint)));
    }
...
/*!
  ...
  This is saved into a new hash containing the filename, the text and the
  priority of that file.
*/
void Component::loadLicenses(const QString &directory,
                             const QHash<QString, QVariant> &licenseHash) {
  ...
  for (it = licenseHash.begin(); it != licenseHash.end(); ++it) {
    ...
    foreach (const QString &lang, QLocale().uiLanguages()) {
      if (QLocale(lang).language() ==
          QLocale::English) // we assume English is the default language
        break;
      ...
      foreach (const QString &locale, QInstaller::localeCandidates(lang)) {
        fileCandidates << QFileInfo(scLocalesArgs.arg(
            directory, fileInfo.baseName(), locale, fileInfo.completeSuffix()));
      }

      auto fInfo =
          std::find_if(fileCandidates.constBegin(), fileCandidates.constEnd(),
                       [](const QFileInfo &file) { return file.exists(); });
    ...
    if (!file.open(QIODevice::ReadOnly)) {
      throw Error(
          tr("Cannot open the requested license file \"%1\": %2.\n\n%3")
              .arg(file.fileName(), file.errorString(), tr(scClearCacheHint)));
    }
...
/*!
  ...
  Operations are added to the list of operations needed to install this
  component.
*/
...
void Component::loadXMLExtractOperations() {
  for (auto &operation : qAsConst(m_operationsList)) {
    if (operation.first == scExtract) {
      // Create hash for Extract operations. Operation has a mandatory extract
      // folder as first argument and optional archive name as second argument.
      ...
      if (operationArgs.count() == 2) {
        ...
        const QString archivePath =
            scInstallerPrefixWithTwoArgs.arg(name()).arg(archiveName);
...
/*!
    Returns a hash that contains the file names, text and priorities of license
...
/*!
    Creates all operations needed to install this component's \a path. \a path
   is a fully qualified filename including the component's name. This method
   gets called from createOperationsForArchive. You can override it by providing
   a method with the same name in the component script.
   ...
   for all files and folders within \a path.
...
void Component::createOperationsForPath(const QString &path) {
  ...
  if (fi.suffix() == scSha1 &&
      QFileInfo(fi.dir(), fi.completeBaseName()).exists())
    return;

  // the script can override this method
  if (!callScriptMethod(scCreateOperationsForPath, QJSValueList() << path)
           .isUndefined())
    return;

  QString target;
  static const QString prefix = scInstallerPrefix;
  target = scTargetDirPlaceholderWithArg.arg(
      path.mid(prefix.length() + name().length()));
...
/*!
    Creates all operations needed to install this component's \a archive. This
   method gets called from createOperations. You can override this method by
   providing a method with the same name in the component script.

    \note If you call this method from a script, it will not call the script's
   method with the same name.

    The default implementation calls createOperationsForPath for everything
   contained in the archive. If \a archive is a compressed archive known to the
   installer system, an Extract operation is created, instead.

    \sa
   {component::createOperationsForArchive}{component.createOperationsForArchive}
*/
void Component::createOperationsForArchive(const QString &archive) {
  ...
  // the script can override this method
  if (!callScriptMethod(scCreateOperationsForArchive, QJSValueList() << archive)
           .isUndefined())
    return;

  QScopedPointer<AbstractArchive> archiveFile(
      ArchiveFactory::instance().create(archive));
  const bool isZip = (archiveFile && archiveFile->open(QIODevice::ReadOnly) &&
                      archiveFile->isSupported());

  if (isZip) {
    // component.xml can override this value
    if (m_archivesHash.contains(archive))
      addOperation(scExtract, QStringList()
                                  << archive << m_archivesHash.value(archive));
    else
...
/*!
    Registers the file or directory at \a path for being removed when this
   component gets uninstalled. In case of a directory, this will be recursive.
   If \a wipe is set to \c true, the directory will also be deleted if it
   contains changes made by the user after installation.

    \sa
   {component::registerPathForUninstallation}{component.registerPathForUninstallation}
*/
...
/*!
    Contains the names of all archives known to this component. Even downloaded
   archives are mapped to the \c{installer://} URL through the used
   QFileEngineHandler during the download process.
*/
...
/*!
    Adds the archive \a path to this component. This can only be called if this
   component was downloaded from an online repository. When adding \a path, it
   will be downloaded from the repository when the installation starts.
...
/*!
    Removes the archive \a path previously added via addDownloadableArchive()
   from this component. This can only be called if this component was downloaded
   from an online repository.

    \sa
   {component::removeDownloadableArchive}{component.removeDownloadableArchive}
...
/*!
    Returns the archives to be downloaded from the online repository before
   installation. Should be called only once when the installation starts.
*/
QStringList Component::downloadableArchives() {
  const QStringList downloadableArchives =
      d->m_downloadableArchivesVariable.split(QInstaller::commaRegExp(),
                                              Qt::SkipEmptyParts);
...
/*!
   ...
   updating, or uninstalling the component.
    ...
    \sa
   {component::addStopProcessForUpdateRequest}{component.addStopProcessForUpdateRequest}
*/
...
/*!
    ...
    \sa
   {component::removeStopProcessForUpdateRequest}{component.removeStopProcessForUpdateRequest}
*/
...
/*!
    A convenience function for adding or removing the request for stopping \a
   process depending on whether \a requested is \c true (add) or \c false
   (remove).

    \sa
   {component::setStopProcessForUpdateRequest}{component.addStopProcessForUpdateReques}
*/
void Component::setStopProcessForUpdateRequest(const QString &process,
                                               bool requested) {
  if (requested)
...
/*!
    The list of processes that need to be closed before installing, updating, or
...
/*!
    Returns the operations needed to install this component. If
   autoCreateOperations() is \c true, createOperations() is called if no
   operations have been automatically created yet.

    The \a mask parameter filters the returned operations by their group.
*/
OperationList
Component::operations(const Operation::OperationGroups &mask) const {
  if (d->m_autoCreateOperations && !d->m_operationsCreated) {
    ...
    if (!d->m_licenses.isEmpty()) {
      d->m_licenseOperation =
          KDUpdater::UpdateOperationFactory::instance().create(scLicense,
                                                               d->m_core);
  ...
  std::copy_if(d->m_operations.begin(), d->m_operations.end(),
               std::back_inserter(operations),
               [&](const Operation *op) { return mask.testFlag(op->group()); });
  return operations;
}

/*!
    Adds \a operation to the list of operations needed to install this
   component.
*/
...
/*!
    Adds \a operation to the list of operations needed to install this
   component. \a operation is executed with elevated rights.
*/
...
/*!
    Returns whether the operations needed to install this component were created
   successfully.
*/
...
Operation *
Component::createOperation(const QString &operationName,
                           ...
                           const QString &parameter7, const QString &parameter8,
                           const QString &parameter9,
...
Operation *Component::createOperation(const QString &operationName,
                                      const QStringList &parameters) {
  Operation *operation = KDUpdater::UpdateOperationFactory::instance().create(
      operationName, d->m_core);
  if (operation == 0) {
    const QMessageBox::StandardButton button = MessageBoxHandler::critical(
        MessageBoxHandler::currentBestSuitParent(),
        QLatin1String("OperationDoesNotExistError"), tr("Error"),
        tr("Error: Operation %1 does not exist.").arg(operationName),
  ...
  // Operation can contain variables which are resolved when performing the
  // operation
...
void Component::markComponentUnstable(Component::UnstableError error,
  ...
  emit packageManagerCore()->unstableComponentFound(
      QLatin1String(metaEnum.valueToKey(error)), errorMessage, this->name());
...
QJSValue Component::callScriptMethod(const QString &methodName,
                                     const QJSValueList &arguments) const {
  QJSValue scriptContext;
  if (!d->m_postScriptContext.isUndefined() &&
      d->m_postScriptContext.property(methodName).isCallable())
    scriptContext = d->m_postScriptContext;
  else
    scriptContext = d->m_scriptContext;
  return d->scriptEngine()->callScriptMethod(scriptContext, methodName,
                                             arguments);
}
...
/*!
    Creates and adds an installation operation for \a operation. Add any number
   of \a parameters. The variables that the parameters contain, such as \c
   @TargetDir@, are replaced with their values.

    Returns \c true if the operation is created; otherwise returns \c false.

    \sa {component::addOperation}{component.addOperation}
*/
bool Component::addOperation(const QString &operation,
                             const QStringList &parameters) {
...
/*!
    Creates and adds the installation operation \a operation. Add any number of
   \a parameters. The variables that the parameters contain, such as \c
   @TargetDir@, are replaced with their values. The operation is executed with
   elevated rights.
...
*/
bool Component::addElevatedOperation(const QString &operation,
                                     const QStringList &parameters) {
  if (Operation *op = createOperation(operation, parameters)) {
...
/*!
    Specifies whether operations should be automatically created when the
   installation starts. This would be done by calling createOperations(). If you
   set this to \c false, it is completely up to the component's script to create
   all operations.

    Returns \c false when component's script will create all operations;
...
*/
bool Component::isSelected() const { return checkState() != Qt::Unchecked; }
...
/*!
    Calls the script method with the validator callback name. Returns \c true if
   the method returns \c true. Always returns \c true if the validator callback
   name is empty.
*/
...
/*!
    Adds the component specified by \a newDependency to the list of
   dependencies. Alternatively, multiple components can be specified by
...
void Component::addDependency(const QString &newDependency) {
  ...
  if (oldDependencies.isEmpty())
    ...
    setValue(scDependencies,
             oldDependencies + scCommaWithSpace + newDependency);
}

/*!
    Returns a list of dependencies defined in the the repository or in the
   package.xml.
*/
...
/*!
    Returns a list of installed components dependencies defined in the
   components.xml.
*/
...
/*!
    Adds the component specified by \a newDependOn to the automatic depend-on
   list. Alternatively, multiple components can be specified by separating each
   with a comma.
...
QStringList Component::autoDependencies() const {
  return d->m_vars.value(scAutoDependOn)
...
*/
void Component::setInstalled() { setValue(scCurrentState, scInstalled); }

/*!
    Determines whether the component comes as an auto dependency. Returns \c
   true if all components in \a componentsToInstall are already installed or
   selected for installation and this component thus needs to be installed as
   well.
...
bool Component::isAutoDependOn(const QSet<QString> &componentsToInstall) const {
  // If there is no auto depend on value or the value is empty, we have nothing
  // todo. The component does not need to be installed as an auto dependency.
  QStringList autoDependOnList = autoDependencies();
  if (autoDependOnList.isEmpty())
    return false;

  // If there is an essential update and autodepend on is not for essential
  // update component, do not add the autodependency to an installed component
  // as essential updates needs to be installed first, otherwise non-essential
  // components will be installed
  if (packageManagerCore()->foundEssentialUpdate()) {
    const QSet<QString> autoDependOnSet(autoDependOnList.begin(),
                                        autoDependOnList.end());
  ...
  const QStringList installedPackages =
  ...
  foreach (const QString &component, components) {
    ...
    if (autoDependOnList.isEmpty()) {
      // If all components in the isAutoDependOn field are already installed or
      // selected for installation, this component needs to be installed as
      // well.
      return true;
    }
  }
...
/*!
    Sets the \a isUpdateAvailable flag to \c true to indicate that the core
   found an update.
...
/*!
    Returns \c true if that component will be changed (update, installation, or
   uninstallation).

    \sa
   {component::componentChangeRequested}{component.componentChangeRequested}
*/
bool Component::componentChangeRequested() {
  return updateRequested() || isSelectedForInstallation() ||
         uninstallationRequested();
}

/*!
    Returns \c true if the component is installed and has a \c ForcedUpdate flag
   set. ForcedUpdate components will be updated together with essential
   components before any other component can be updated or installed.
...
*/
void Component::setUninstalled() { setValue(scCurrentState, scUninstalled); }
...
/*!
    Sets this component, all its child components and all the components
   depending on this component unstable with the error code \a error and message
   \a errorMessage.
*/
void Component::setUnstable(Component::UnstableError error,
                            const QString &errorMessage) {
  ...
  foreach (Component *dependency, dependencies) {
    dependency->markComponentUnstable(
        UnstableError::DepencyToUnstable,
    ...
    foreach (Component *descendant, dependency->descendantComponents()) {
      descendant->markComponentUnstable(
          UnstableError::DescendantOfUnstable,
...
/*!
   ...
   empty QUrl.
*/
QUrl Component::repositoryUrl() const { return d->m_repositoryUrl; }

/*!
    Sets this component's repository URL as \a url.
*/
void Component::setRepositoryUrl(const QUrl &url) { d->m_repositoryUrl = url; }

/*!
    Returns the path to the local directory where the component is temporarily
   stored.
*/
QString Component::localTempPath() const { return d->m_localTempPath; }
...
void Component::updateModelData(const QString &key, const QString &data) {
  if (key == scVirtual) {
    setData(data.toLower() == scTrue ? d->m_core->virtualComponentsFont()
                                     : QFont(),
            Qt::FontRole);
  ...
  if (key == scUpdateText || key == scDescription) {
    ...
    if (!d->m_core->isUpdater() || updateInfo.isEmpty()) {
      tooltipText = QString::fromLatin1("<html><body>%1</body></html>")
                        .arg(d->m_vars.value(scDescription));
    } else {
    ...
    if (isUnstable()) {
      tooltipText +=
          scBr + tr("There was an error loading the selected component. "
                    "This component cannot be installed.");
    }
    static const QRegularExpression externalLinkRegexp(
        QLatin1String("{external-link}='(.*?)'"));
    static const QLatin1String externalLinkElement(
        QLatin1String("<a href=\"\\1\">\\1</a>"));
    // replace {external-link}='' fields in component description with proper
    // link tags
