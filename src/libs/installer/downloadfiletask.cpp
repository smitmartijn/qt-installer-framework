...
AuthenticationRequiredException::AuthenticationRequiredException(
    Type type, const QString &message)
    : TaskException(message), m_type(type) {}

Downloader::Downloader() : m_finished(0) {
...
void Downloader::download(QFutureInterface<FileTaskResult> &fi,
                          const QList<FileTaskItem> &items,
...
void Downloader::onReadyRead() {
  ...
  if (!data.file) {
    ...
    if (file->exists() && (!QFileInfo(file->fileName()).isFile())) {
      m_futureInterface->reportException(
          TaskException(tr("Target file \"%1\" already exists "
                           "but is not a file.")
                            .arg(file->fileName())));
      return;
    }
    ...
    if (!file->open(QIODevice::WriteOnly | QIODevice::Truncate)) {
      ...
      m_futureInterface->reportException(
          TaskException(tr("Cannot open file \"%1\" for writing: %2")
  ...
  if (!data.file->isOpen()) {
    //: %2 is a sentence describing the error.
    m_futureInterface->reportException(
        TaskException(tr("File \"%1\" not open for writing: %2")
                          .arg(QDir::toNativeSeparators(data.file->fileName()),
  ...
  while (reply->bytesAvailable()) {
    ...
    while (written < read) {
      const qint64 toWrite =
          data.file->write(buffer.constData() + written, read - written);
      if (toWrite < 0) {
        //: %2 is a sentence describing the error.
        m_futureInterface->reportException(TaskException(
...
void Downloader::onFinished(QNetworkReply *reply) {
  ...
  if (!m_futureInterface->isCanceled()) {
    if (reply->attribute(QNetworkRequest::RedirectionTargetAttribute)
      ...
      if (!redirects.contains(url)) {
        ...
        m_futureInterface->reportException(TaskException(
            tr("Redirect loop detected for \"%1\".").arg(url.toString())));
        return;
      }
    }
  }
  ...
  const QByteArray expectedCheckSum =
      data.taskItem.value(TaskRole::Checksum).toByteArray();
  bool checksumMismatch = false;
  if (!expectedCheckSum.isEmpty()) {
    if (expectedCheckSum != data.observer->checkSum().toHex())
      checksumMismatch = true;
  }
  m_futureInterface->reportResult(FileTaskResult(
      filename, data.observer->checkSum(), data.taskItem, checksumMismatch));
...
void Downloader::onError(QNetworkReply::NetworkError error) {
  ...
  if (reply) {
    ...
    // Do not throw error if Updates.xml not found. The repository might be
    // removed with RepositoryUpdate in Updates.xml later.
    //: %2 is a sentence describing the error
    if (data.taskItem.source().contains(QLatin1String("Updates.xml"),
                                        Qt::CaseInsensitive)) {
      qCWarning(QInstaller::lcServer)
          << QString::fromLatin1("Network error while downloading '%1': %2.")
                 .arg(data.taskItem.source(), reply->errorString());
    } else {
      m_futureInterface->reportException(TaskException(
          tr("Network error while downloading '%1': %2.")
              .arg(data.taskItem.source(), reply->errorString())));
    }
  } else {
    //: %1 is a sentence describing the error
    m_futureInterface->reportException(TaskException(
        tr("Unknown network error while downloading \"%1\".").arg(error)));
  }
}
...
void Downloader::onAuthenticationRequired(QNetworkReply *reply,
  ...
  const QAuthenticator auth =
      item->value(TaskRole::Authenticator).value<QAuthenticator>();
  if (auth.user().isEmpty()) {
    AuthenticationRequiredException e(
        AuthenticationRequiredException::Type::Server,
        QCoreApplication::translate("AuthenticationRequiredException",
                                    "%1 at %2")
            .arg(authenticator->realm(), reply->url().host()));
    item->insert(TaskRole::Authenticator,
                 QVariant::fromValue(QAuthenticator(*authenticator)));
    ...
    item->insert(TaskRole::Authenticator,
                 QVariant()); // clear so we fail on next call
  }
}

void Downloader::onProxyAuthenticationRequired(const QNetworkProxy &proxy,
                                               QAuthenticator *) {
  // Report to GUI thread.
  // (MetadataJob will ask for username/password, and restart the download ...)
  AuthenticationRequiredException e(
      AuthenticationRequiredException::Type::Proxy,
      QCoreApplication::translate("AuthenticationRequiredException",
...
/*!
    ...
    Canceling from the outside will not get noticed if we are waiting on a
   connection that does not create any events. QNam will drop after 45 seconds,
   though the user might have canceled the download before. In that case we
   block until the QNam timeout is reached, worst case resulting in deadlock
   while the application is shutting down at the same time.
*/
void Downloader::onTimeout() {
  if (testCanceled()) {
    // Inject exception, we can't use QFuturInterface::reportException() as the
    // exception store is "frozen" once cancel was called. On the other hand,
    // client code could use QFutureWatcherBase::isCanceled() or
    // QFuture::isCanceled() to check for canceled futures.
    m_futureInterface->exceptionStore().setException(
        TaskException(tr("Network transfers canceled.")));
    m_futureInterface->reportFinished();
    emit finished();
  }
}
...
bool Downloader::testCanceled() {
  ...
  if (m_futureInterface->isPaused()) {
    ...
    m_futureInterface->reportException(TaskException(
        tr("Pause and resume not supported by network transfers.")));
  }
...
QNetworkReply *Downloader::startDownload(const FileTaskItem &item) {
  ...
  if (!source.isValid()) {
    //: %2 is a sentence describing the error
    m_futureInterface->reportException(
        TaskException(tr("Invalid source URL \"%1\": %2")
  ...
  connect(reply, &QNetworkReply::errorOccurred, this, &Downloader::onError);
#ifndef QT_NO_SSL
...
void DownloadFileTask::setProxyFactory(
    KDUpdater::FileDownloaderProxyFactory *factory) {
  ...
  if (!m_authenticator.isNull()) {
    for (int i = 0; i < items.count(); ++i) {
      if (items.at(i).value(TaskRole::Authenticator).isNull())
        items[i].insert(TaskRole::Authenticator,
                        QVariant::fromValue(m_authenticator));
    }
  }
  downloader.download(fi, items,
                      (m_proxyFactory.isNull() ? 0 : m_proxyFactory->clone()));
  el.exec(); // That's tricky here, we need to run our own event loop to keep
             // QNAM working.
}
