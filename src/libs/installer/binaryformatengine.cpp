...
public:
  StringListIterator(const QStringList &list, QDir::Filters filters,
                     const QStringList &nameFilters)
      : QAbstractFileEngineIterator(filters, nameFilters), list(list),
        index(-1) {}

  bool hasNext() const { return index < list.size() - 1; }
...
/*!
    ...
    \brief The BinaryFormatEngine class is the default file engine for accessing
   resource collections and resource files.
*/
...
BinaryFormatEngine::BinaryFormatEngine(
    const QHash<QByteArray, ResourceCollection> &collections,
    const QString &fileName)
    : m_resource(nullptr), m_collections(collections) {
...
/*!
    ...
    Sets the file engine's file name to \a file. This is the file that the rest
   of the virtual functions will operate on.
*/
...
BinaryFormatEngine::fileFlags(FileFlags type) const {
  FileFlags result;
...
BinaryFormatEngine::beginEntryList(QDir::Filters filters,
...
*/
QStringList
BinaryFormatEngine::entryList(QDir::Filters filters,
  ...
  result.removeAll(QString()); // Remove empty names, will crash while using
                               // directory iterator.
  ...
  for (const QString &i : filterNames) {
    regexps.append(
        QRegularExpression(QRegularExpression::wildcardToRegularExpression(i),
