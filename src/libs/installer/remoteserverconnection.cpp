...
/*!
    ...
    \internal
    \brief The RemoteServerReply class is used for sending a reply to a local
   socket.

           The class ensures exactly one reply is sent in the lifetime of an
   object instantiated from this class. If the calling client has not explicitly
   sent a reply, a default reply is automatically sent on destruction.
*/
...
*/
template <typename T> void RemoteServerReply::send(const T &data) {
...
RemoteServerConnection::RemoteServerConnection(qintptr socketDescriptor,
                                               const QString &key,
                                               QObject *parent)
    : QThread(parent), m_socketDescriptor(socketDescriptor),
      m_authorizationKey(key), m_process(nullptr), m_engine(nullptr),
      m_archive(nullptr), m_processSignalReceiver(nullptr),
      m_archiveSignalReceiver(nullptr) {
  setObjectName(
      QString::fromLatin1("RemoteServerConnection(%1)").arg(socketDescriptor));
}
    ...
    if (authorized && command == QLatin1String(Protocol::Shutdown)) {
      ...
      if (command == QLatin1String(Protocol::Create)) {
        ...
        if (type == QLatin1String(Protocol::QSettings)) {
          ...
          if (fileName.toString().isEmpty()) {
            settings.reset(new PermissionSettings(
                QSettings::Format(format.toInt()),
                QSettings::Scope(scope.toInt()), organization.toString(),
                application.toString()));
          } else {
            settings.reset(new PermissionSettings(
                fileName.toString(), QSettings::Format(format.toInt())));
          }
          ...
          Q_ASSERT_X(false, Q_FUNC_INFO,
                     "No compatible archive handler exists for protocol.");
#endif
        } else {
          qCDebug(QInstaller::lcServer)
              << "Unknown type for Create command:" << type;
        }
        continue;
      }
      ...
      if (command == QLatin1String(Protocol::GetQProcessSignals)) {
        ...
        continue;
                 ...
                 QLatin1String(Protocol::GetAbstractArchiveSignals)) {
#ifdef IFW_LIBARCHIVE
      ...
      if (command.startsWith(QLatin1String(Protocol::QProcess))) {
        ...
        handleQSettings(&reply, command, stream, settings.data());
                     ...
                     QLatin1String(Protocol::QAbstractFileEngine))) {
...
void RemoteServerConnection::handleQProcess(RemoteServerReply *reply,
  ...
  if (command == QLatin1String(Protocol::QProcessCloseWriteChannel)) {
    ...
    reply->send(m_process->readAll());
             ...
             QLatin1String(Protocol::QProcessReadAllStandardOutput)) {
    ...
    qint64 pid = -1;
    bool success =
        QInstaller::startDetached(program, arguments, workingDirectory, &pid);
    ...
    data >> processChannel;
    m_process->setReadChannel(
        static_cast<QProcess::ProcessChannel>(processChannel));
    ...
    reply->send(static_cast<qint32>(m_process->processChannelMode()));
             ...
             QLatin1String(Protocol::QProcessSetProcessChannelMode)) {
    qint32 processChannel;
    data >> processChannel;
    m_process->setProcessChannelMode(
        static_cast<QProcess::ProcessChannelMode>(processChannel));
  }
#ifdef Q_OS_WIN
...
void RemoteServerConnection::handleQSettings(RemoteServerReply *reply,
                                             const QString &command,
                                             QDataStream &data,
...
void RemoteServerConnection::handleQFSFileEngine(RemoteServerReply *reply,
                                                 const QString &command,
                                                 QDataStream &data) {
  if (command == QLatin1String(Protocol::QAbstractFileEngineAtEnd)) {
    reply->send(m_engine->atEnd());
  } else if (command ==
             QLatin1String(Protocol::QAbstractFileEngineCaseSensitive)) {
    ...
    reply->send(m_engine->handle());
             ...
             QLatin1String(Protocol::QAbstractFileEngineIsRelativePath)) {
    reply->send(m_engine->isRelativePath());
  } else if (command ==
             QLatin1String(Protocol::QAbstractFileEngineIsSequential)) {
    ...
    data >> owner;
    reply->send(
        m_engine->owner(static_cast<QAbstractFileEngine::FileOwner>(owner)));
  } else if (command == QLatin1String(Protocol::QAbstractFileEngineOwnerId)) {
    qint32 owner;
    data >> owner;
    reply->send(
        m_engine->ownerId(static_cast<QAbstractFileEngine::FileOwner>(owner)));
    ...
    reply->send(m_engine->seek(offset));
             ...
             QLatin1String(Protocol::QAbstractFileEngineSetFileName)) {
    QString fileName;
    data >> fileName;
    m_engine->setFileName(fileName);
  } else if (command ==
             QLatin1String(Protocol::QAbstractFileEngineSetPermissions)) {
    uint perms;
    data >> perms;
    ...
    reply->send(m_engine->size());
              ...
              QLatin1String(Protocol::QAbstractFileEngineSupportsExtension)) ||
             (command ==
              QLatin1String(Protocol::QAbstractFileEngineExtension))) {
    ...
    reply->send(m_engine->write(content.data(), content.size()));
             ...
             QLatin1String(Protocol::QAbstractFileEngineSyncToDisk)) {
    reply->send(m_engine->syncToDisk());
  } else if (command ==
             QLatin1String(Protocol::QAbstractFileEngineRenameOverwrite)) {
    QString newFilename;
    data >> newFilename;
    ...
    qint32 filetime;
    data >> filetime;
    reply->send(m_engine->fileTime(
        static_cast<QAbstractFileEngine::FileTime>(filetime)));
  } else if (!command.isEmpty()) {
    qCDebug(QInstaller::lcServer)
        << "Unknown QAbstractFileEngine command:" << command;
  }
}

void RemoteServerConnection::handleArchive(RemoteServerReply *reply,
                                           const QString &command,
                                           QDataStream &data) {
#ifdef IFW_LIBARCHIVE
  ...
  if (command == QLatin1String(Protocol::AbstractArchiveOpen)) {
    ...
    reply->send(archive->isSupported());
             ...
             QLatin1String(Protocol::AbstractArchiveSetCompressionLevel)) {
    qint32 level;
    data >> level;
    archive->setCompressionLevel(
        static_cast<AbstractArchive::CompressionLevel>(level));
  } else if (command == QLatin1String(Protocol::AbstractArchiveAddDataBlock)) {
    QByteArray buff;
    data >> buff;
    archive->workerAddDataBlock(buff);
  } else if (command ==
             QLatin1String(Protocol::AbstractArchiveSetClientDataAtEnd)) {
    archive->workerSetDataAtEnd();
  } else if (command ==
             QLatin1String(Protocol::AbstractArchiveSetFilePosition)) {
    qint64 pos;
    data >> pos;
    ...
    qCDebug(QInstaller::lcServer)
        << "Unknown AbstractArchive command:" << command;
  }
#else
  Q_ASSERT_X(false, Q_FUNC_INFO,
             "No compatible archive handler exists for protocol.");
