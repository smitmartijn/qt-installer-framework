...
static QStringList
checkRunningProcessesFromList(const QStringList &processList) {
  ...
  foreach (const QString &process, processList) {
    if (!process.isEmpty() &&
        PackageManagerCorePrivate::isProcessRunning(process, allProcesses))
      stillRunningProcesses.append(process);
  }
  return stillRunningProcesses;
}

static void deferredRename(const QString &oldName, const QString &newName,
                           bool restart = false) {
#ifdef Q_OS_WIN
...
PackageManagerCorePrivate::PackageManagerCorePrivate(PackageManagerCore *core)
    : m_updateFinder(nullptr),
      ...
      m_status(PackageManagerCore::Unfinished), m_needsHardRestart(false),
      m_testChecksum(false),
      m_launchedAsRoot(AdminAuthorization::hasAdminRights()),
      m_completeUninstall(false), m_needToWriteMaintenanceTool(false),
      m_dependsOnLocalInstallerBinary(false), m_core(core), m_updates(false),
      m_repoFetched(false), m_updateSourcesAdded(false),
      m_magicBinaryMarker(0) // initialize with pseudo marker
      ,
      m_magicMarkerSupplement(BinaryContent::Default),
      m_componentScriptEngine(nullptr), m_controlScriptEngine(nullptr),
      m_installerCalculator(nullptr), m_uninstallerCalculator(nullptr),
      m_proxyFactory(nullptr), m_defaultModel(nullptr), m_updaterModel(nullptr),
      m_guiObject(nullptr), m_remoteFileEngineHandler(nullptr),
      m_foundEssentialUpdate(false), m_commandLineInstance(false),
      m_defaultInstall(false), m_userSetBinaryMarker(false),
      m_checkAvailableSpace(true), m_autoAcceptLicenses(false),
      m_disableWriteMaintenanceTool(false), m_autoConfirmCommand(false),
      m_datFileName(QString()) {}

PackageManagerCorePrivate::PackageManagerCorePrivate(
    PackageManagerCore *core, qint64 magicInstallerMaker,
  ...
  foreach (const OperationBlob &operation, performedOperations) {
    QScopedPointer<QInstaller::Operation> op(
        KDUpdater::UpdateOperationFactory::instance().create(operation.name,
                                                             core));
    if (op.isNull()) {
      qCWarning(QInstaller::lcInstallerInstallLog)
          << "Failed to load unknown operation" << operation.name;
      continue;
    }

    if (!op->fromXml(operation.xml)) {
      qCWarning(QInstaller::lcInstallerInstallLog)
          << "Failed to load XML for operation" << operation.name;
      continue;
    }
    m_performedOperationsOld.append(op.take());
  }

  connect(this, &PackageManagerCorePrivate::installationStarted, m_core,
          &PackageManagerCore::installationStarted);
  connect(this, &PackageManagerCorePrivate::installationFinished, m_core,
          &PackageManagerCore::installationFinished);
  connect(this, &PackageManagerCorePrivate::uninstallationStarted, m_core,
          &PackageManagerCore::uninstallationStarted);
  connect(this, &PackageManagerCorePrivate::uninstallationFinished, m_core,
          &PackageManagerCore::uninstallationFinished);
  connect(this, &PackageManagerCorePrivate::offlineGenerationStarted, m_core,
          &PackageManagerCore::offlineGenerationStarted);
  connect(this, &PackageManagerCorePrivate::offlineGenerationFinished, m_core,
          &PackageManagerCore::offlineGenerationFinished);
}
...
PackageManagerCorePrivate::~PackageManagerCorePrivate() {
  ...
  // at the moment the tabcontroller deletes the m_gui, this needs to be changed
  // in the future delete m_gui;
}

/*
    Return true, if a process with \a name is running. On Windows, comparison is
   case-insensitive.
*/
/* static */
bool PackageManagerCorePrivate::isProcessRunning(
    const QString &name, const QList<ProcessInfo> &processes) {
  ...
  for (it = processes.constBegin(); it != processes.constEnd(); ++it) {
    ...
    if (it->name.toLower() == QDir::toNativeSeparators(name.toLower()))
      return true;
    const QFileInfo fi(it->name);
    if (fi.fileName().toLower() == name.toLower() ||
        fi.baseName().toLower() == name.toLower())
      return true;
#endif
  }
  return false;
...
/* static */
bool PackageManagerCorePrivate::performOperationThreaded(
  ...
  QEventLoop loop;
  QObject::connect(&futureWatcher, &decltype(futureWatcher)::finished, &loop,
...
QString PackageManagerCorePrivate::configurationFileName() const {
  return m_core->value(scTargetConfigurationFile,
                       QLatin1String("components.xml"));
}

QString PackageManagerCorePrivate::componentsXmlPath() const {
  return QDir::toNativeSeparators(
      QDir(QDir::cleanPath(targetDir()))
          .absoluteFilePath(configurationFileName()));
}

bool PackageManagerCorePrivate::buildComponentTree(
    QHash<QString, Component *> &components, bool loadScript) {
  try {
    ...
    // after everything is set up, load the scripts if needed and create helper
    // hashes for autodependency and dependency components for quicker search
    // later
    ...
    foreach (QInstaller::Component *component, components) {
      // set the checked state for all components without child (means without
      // tristate) set checked state also for installed virtual tristate
      // componets as otherwise those will be uninstalled
      if (component->isCheckable() && !component->isTristate()) {
        if (component->isDefault() && isInstaller())
          ...
          component->setCheckState(Qt::Checked);
                 ...
                 component->isTristate()) {
        component->setCheckState(Qt::Checked);
      }
    }

    std::sort(m_rootComponents.begin(), m_rootComponents.end(),
    ...
    if (installerCalculator()->solve(components.values()) == false) {
      ...
      MessageBoxHandler::critical(
          MessageBoxHandler::currentBestSuitParent(), QLatin1String("Error"),
    ...
    if (LoggingHandler::instance().verboseLevel() == LoggingHandler::Detailed) {
      foreach (QInstaller::Component *component, components) {
        const QStringList warnings =
            ComponentChecker::checkComponent(component);
...
bool PackageManagerCorePrivate::loadComponentScripts(const T &components,
  ...
  for (auto *component : components) {
    ...
    const int currentProgress =
        qRound(double(loadedComponents) / components.count() * 100);
...
template bool
PackageManagerCorePrivate::loadComponentScripts<QList<Component *>>(
    const QList<Component *> &, const bool);
template bool
PackageManagerCorePrivate::loadComponentScripts<QHash<QString, Component *>>(
...
void PackageManagerCorePrivate::clearUpdaterComponentLists() {
  ...
  QSet<Component *> usedComponents(m_updaterComponents.begin(),
                                   m_updaterComponents.end());
  usedComponents.unite(QSet<Component *>(m_updaterComponentsDeps.begin(),
                                         m_updaterComponentsDeps.end()));

  const QList<QPair<Component *, Component *>> list =
      m_componentsToReplaceUpdaterMode.values();
  for (int i = 0; i < list.count(); ++i) {
    if (usedComponents.contains(list.at(i).second))
      qCWarning(QInstaller::lcDeveloperBuild)
          << "a replacement was already in the list - is that correct?";
    else
...
QList<Component *> &
PackageManagerCorePrivate::replacementDependencyComponents() {
  return (!isUpdater()) ? m_rootDependencyReplacements
                        : m_updaterDependencyReplacements;
}

QHash<QString, QPair<Component *, Component *>> &
PackageManagerCorePrivate::componentsToReplace() {
  return (!isUpdater()) ? m_componentsToReplaceAllMode
                        : m_componentsToReplaceUpdaterMode;
}
...
InstallerCalculator *PackageManagerCorePrivate::installerCalculator() const {
  if (!m_installerCalculator) {
    PackageManagerCorePrivate *const pmcp =
        const_cast<PackageManagerCorePrivate *>(this);
    pmcp->m_installerCalculator =
        new InstallerCalculator(m_core, pmcp->m_autoDependencyComponentHash);
  }
...
PackageManagerCorePrivate::uninstallerCalculator() const {
  if (!m_uninstallerCalculator) {
    ...
    foreach (const QString &name, pmcp->localInstalledPackages().keys()) {
      if (Component *component = m_core->componentByName(
              PackageManagerCore::checkableName(name))) {
        if (!component->uninstallationRequested())
          installedComponents.append(component);
      }
    }

    pmcp->m_uninstallerCalculator = new UninstallerCalculator(
        m_core, pmcp->m_autoDependencyComponentHash,
        pmcp->m_localDependencyComponentHash, pmcp->m_localVirtualComponents);
  }
...
void PackageManagerCorePrivate::initialize(
  ...
  if (m_launchedAsRoot && isInstaller())
    m_data.setValue(scTargetDir,
                    replaceVariables(m_data.settings().adminTargetDir()));
#endif
  ...
  if (m_core->isCommandLineInstance() && m_core->isInstaller()) {
    ...
    QString startMenuDir = m_core->value(
        scStartMenuDir, m_core->value(QLatin1String("ProductName")));
    m_data.setValue(scStartMenuDir,
                    startMenuPath + QDir::separator() + startMenuDir);
  }
#endif
  ...
  if (isInstaller() || m_localPackageHub->applicationName().isEmpty()) {
    // TODO: this seems to be wrong, we should ask for ProductName defaulting to
    // applicationName...
  ...
  if (isInstaller() || m_localPackageHub->applicationVersion().isEmpty())
    m_localPackageHub->setApplicationVersion(
        QLatin1String(QUOTE(IFW_REPOSITORY_FORMAT_VERSION)));

  if (isInstaller())
    m_packageSources.insert(
        PackageSource(QUrl(QLatin1String("resource://metadata/")), 1));

  m_metadataJob.disconnect();
  m_metadataJob.setAutoDelete(false);
  m_metadataJob.setPackageManagerCore(m_core);

  connect(&m_metadataJob, &Job::infoMessage, this,
          &PackageManagerCorePrivate::infoMessage);
  connect(&m_metadataJob, &Job::progress, this,
          &PackageManagerCorePrivate::infoProgress);
  connect(&m_metadataJob, &Job::totalProgress, this,
          &PackageManagerCorePrivate::totalProgress);
  KDUpdater::FileDownloaderFactory::instance().setProxyFactory(
      m_core->proxyFactory());
}
...
bool PackageManagerCorePrivate::isOfflineOnly() const {
  ...
  QSettings confInternal(QLatin1String(":/config/config-internal.ini"),
                         QSettings::IniFormat);
...
bool PackageManagerCorePrivate::statusCanceledOrFailed() const {
  return m_status == PackageManagerCore::Canceled ||
         m_status == PackageManagerCore::Failure;
}
...
QByteArray
PackageManagerCorePrivate::replaceVariables(const QByteArray &ba) const {
 ...
 */
Operation *
PackageManagerCorePrivate::createOwnedOperation(const QString &type) {
  m_ownedOperations.append(
      KDUpdater::UpdateOperationFactory::instance().create(type, m_core));
  return m_ownedOperations.last();
}

/*!
    \internal
    Removes \a operation from the operations owned by the installer, returns the
   very same operation if the operation was found, otherwise 0.
 */
...
QString PackageManagerCorePrivate::maintenanceToolAliasPath() const {
  ...
  const bool isRoot = (AdminAuthorization::hasAdminRights() ||
                       RemoteClient::instance().isActive());
  const QString applicationsDir =
      m_core->value(isRoot ? QLatin1String("ApplicationsDir")
                           : QLatin1String("ApplicationsDirUser"));
  QString maintenanceToolAlias =
      QString::fromLatin1("%1/%2").arg(applicationsDir, aliasName);
...
QString PackageManagerCorePrivate::offlineBinaryName() const {
  QString filename =
      m_core->value(scOfflineBinaryName,
                    qApp->applicationName() + QLatin1String("_offline-") +
...
QString PackageManagerCorePrivate::datFileName() {
  if (m_datFileName.isEmpty())
    m_datFileName = targetDir() + QLatin1Char('/') +
                    m_data.settings().maintenanceToolName() +
                    QLatin1String(".dat");
...
static QSet<Repository> readRepositories(QXmlStreamReader &reader,
                                         bool isDefault) {
  QSet<Repository> set;
  while (reader.readNextStartElement()) {
...
void PackageManagerCorePrivate::writeMaintenanceConfigFiles() {
  ...
  const QString iniPath = targetDir() + QLatin1Char('/') +
                          m_data.settings().maintenanceToolIniFile();

  QVariantHash
      variables; // Do not change to QVariantMap! Breaks existing .ini files,
  // cause the variant types do not match while restoring the variables from the
  // file.
  ...
  foreach (const QString &key, m_data.keys()) {
    if (key == scRunProgramDescription || key == scRunProgram ||
        key == scRunProgramArguments)
      continue;
    QVariant value = m_data.value(key);
    if (value.canConvert(QVariant::String))
      value = replacePath(value.toString(), targetDir(),
                          QLatin1String(scRelocatable));
  ...
  cfg.setValue(QLatin1String("FilesForDelayedDeletion"),
               m_filesForDelayedDeletion);

  cfg.sync();
  if (cfg.status() != QSettingsWrapper::NoError) {
    const QString reason = cfg.status() == QSettingsWrapper::AccessError
                               ? tr("Access error")
                               : tr("Format error");
    throw Error(tr("Cannot write installer configuration to %1: %2")
                    .arg(iniPath, reason));
  }
  ...
  if (file.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
    ...
    writer.writeTextElement(QLatin1String("ProxyType"),
                            QString::number(m_data.settings().proxyType()));
    ...
    writer.writeTextElement(QLatin1String("Port"),
                            QString::number(ftpProxy.port()));
    ...
    foreach (const Repository &repo, m_data.settings().userRepositories()) {
      ...
      writer.writeTextElement(QLatin1String("Enabled"),
                              QString::number(repo.isEnabled()));
      writer.writeEndElement();
    }
    writer.writeEndElement();
    writer.writeTextElement(QLatin1String("LocalCachePath"),
                            m_data.settings().localCachePath());
    writer.writeEndElement();
  }
  setDefaultFilePermissions(&file, DefaultFilePermissions::NonExecutable);
}

void PackageManagerCorePrivate::readMaintenanceConfigFiles(
    const QString &targetDir) {
  QSettingsWrapper cfg(targetDir + QLatin1Char('/') +
                           m_data.settings().maintenanceToolIniFile(),
                       QSettings::IniFormat);
  const QVariantHash v =
      cfg.value(QLatin1String("Variables")).toHash(); // Do not change to
  // QVariantMap! Breaks reading from existing .ini files, cause the variant
  // types do not match.
  for (QVariantHash::const_iterator it = v.constBegin(); it != v.constEnd();
       ++it) {
    ...
    m_data.setValue(it.key(),
                    replacePath(it.value().toString(),
                                QLatin1String(scRelocatable), targetDir));
  }
  ...
  m_filesForDelayedDeletion =
      cfg.value(QLatin1String("FilesForDelayedDeletion")).toStringList();
      ...
      if (reader.name() == QLatin1String("Network")) {
        while (reader.readNextStartElement()) {
          ...
          if (name == QLatin1String("Ftp")) {
            ...
            m_data.settings().addUserRepositories(
                readRepositories(reader, false));
          } else if (name == QLatin1String("ProxyType")) {
            m_data.settings().setProxyType(
                Settings::ProxyType(reader.readElementText().toInt()));
...
void PackageManagerCorePrivate::callBeginInstallation(
    const QList<Component *> &componentList) {
  foreach (Component *component, componentList)
    component->beginInstallation();
}

void PackageManagerCorePrivate::stopProcessesForUpdates(
    const QList<Component *> &components) {
  QStringList processList;
  foreach (const Component *component, components)
    processList << m_core->replaceVariables(
        component->stopProcessForUpdateRequests());

  std::sort(processList.begin(), processList.end());
  processList.erase(std::unique(processList.begin(), processList.end()),
                    processList.end());
  ...
  while (true) {
    ...
    const QMessageBox::StandardButton button = MessageBoxHandler::warning(
        MessageBoxHandler::currentBestSuitParent(),
        QLatin1String("stopProcessesForUpdates"), tr("Stop Processes"),
        tr("These processes "
...
int PackageManagerCorePrivate::countProgressOperations(
  ...
  foreach (Operation *operation, operations) {
    if (QObject *operationObject = dynamic_cast<QObject *>(operation)) {
      ...
      if (mo->indexOfSignal(
              QMetaObject::normalizedSignature("progressChanged(double)")) > -1)
        operationCount++;
    }
  }
...
void PackageManagerCorePrivate::connectOperationToInstaller(
    Operation *const operation, double operationPartSize) {
  Q_ASSERT(operationPartSize);
  QObject *const operationObject = dynamic_cast<QObject *>(operation);
  if (operationObject != nullptr) {
    const QMetaObject *const mo = operationObject->metaObject();
    if (mo->indexOfSignal(QMetaObject::normalizedSignature(
            "outputTextChanged(QString)")) > -1) {
      connect(operationObject, SIGNAL(outputTextChanged(QString)),
              ProgressCoordinator::instance(),
              SLOT(emitDetailTextChanged(QString)));
    }

    if (mo->indexOfSlot(QMetaObject::normalizedSignature("cancelOperation()")) >
        -1)
      connect(m_core, SIGNAL(installationInterrupted()), operationObject,
              SLOT(cancelOperation()));

    if (mo->indexOfSignal(
            QMetaObject::normalizedSignature("progressChanged(double)")) > -1) {
      ProgressCoordinator::instance()->registerPartProgress(
          operationObject, SIGNAL(progressChanged(double)), operationPartSize);
    }
  }
}

Operation *
PackageManagerCorePrivate::createPathOperation(const QFileInfo &fileInfo,
                                               const QString &componentName) {
  const bool isDir = fileInfo.isDir();
  // create an operation with the dir/ file as target, it will get deleted on
  // undo
  Operation *operation =
      createOwnedOperation(QLatin1String(isDir ? "Mkdir" : "Copy"));
  if (isDir)
    operation->setValue(QLatin1String("createddir"),
...
/*!
    This creates fake operations which remove stuff which was registered for
   uninstallation afterwards
*/
void PackageManagerCorePrivate::registerPathsForUninstallation(
    const QList<QPair<QString, bool>> &pathsForUninstallation,
    const QString &componentName) {
  if (pathsForUninstallation.isEmpty())
    return;

  QList<QPair<QString, bool>>::const_iterator it;
  for (it = pathsForUninstallation.begin(); it != pathsForUninstallation.end();
       ++it) {
    const bool wipe = it->second;
    const QString path = replaceVariables(it->first);

    const QFileInfo fi(path);
    // create a copy operation with the file as target -> it will get deleted on
    // undo
    ...
    if (fi.isDir() && !wipe) {
      QDirIterator dirIt(path,
                         QDir::Hidden | QDir::AllEntries | QDir::System |
                             QDir::NoDotAndDotDot,
...
void PackageManagerCorePrivate::writeMaintenanceToolBinary(
    QFile *const input, qint64 size, bool writeBinaryLayout) {
  QString maintenanceToolRenamedName =
      maintenanceToolName() + QLatin1String(".new");
  qCDebug(QInstaller::lcInstallerInstallLog)
      << "Writing maintenance tool:" << maintenanceToolRenamedName;
  ProgressCoordinator::instance()->emitLabelAndDetailTextChanged(
      tr("Writing maintenance tool."));

  QFile out(generateTemporaryFileName());
  QInstaller::openForWrite(&out); // throws an exception in case of error

  if (!input->seek(0))
    throw Error(tr("Failed to seek in file %1: %2")
  ...
  if (writeBinaryLayout) {
...
#endif
    // It's a bit odd to have only the magic in the data file, but this
    // simplifies other code a lot (since installers don't have any appended
    // data either)
...

    {
      ...
      if (dummy.exists() && !dummy.remove()) {
        throw Error(tr("Cannot remove data file \"%1\": %2")
                        .arg(dummy.fileName(), dummy.errorString()));
      }
    }

    if (!dataOut.rename(
            resourcePath.filePath(QLatin1String("installer.dat")))) {
      throw Error(tr("Cannot write maintenance tool data to %1: %2")
                      .arg(dataOut.fileName(), dataOut.errorString()));
    }
  ...
  if (!out.copy(maintenanceToolRenamedName)) {
    throw Error(tr("Cannot write maintenance tool to \"%1\": %2")
                    .arg(maintenanceToolRenamedName, out.errorString()));
  }
  ...
  if (setDefaultFilePermissions(&mt, DefaultFilePermissions::Executable))
    ...
    qCWarning(QInstaller::lcInstallerInstallLog)
        << "Failed to write permissions for maintenance tool.";

  if (out.exists() && !out.remove()) {
    qCWarning(QInstaller::lcInstallerInstallLog)
        << tr("Cannot remove temporary data file \"%1\": %2")
               .arg(out.fileName(), out.errorString());
  }
}

void PackageManagerCorePrivate::writeMaintenanceToolBinaryData(
    QFileDevice *output, QFile *const input,
  ...
  const QString newDefaultResource =
      m_core->value(QString::fromLatin1("DefaultResourceReplacement"));
  if (!newDefaultResource.isEmpty()) {
    QFile file(newDefaultResource);
    if (file.open(QIODevice::ReadOnly)) {
      resourceSegments.append(
          Range<qint64>::fromStartAndLength(output->pos(), file.size()));
      QInstaller::appendData(output, &file, file.size());
      existingResourceSegments.remove(0);

      file.remove(); // clear all possible leftovers
      m_core->setValue(QString::fromLatin1("DefaultResourceReplacement"),
                       QString());
    } else {
      qCWarning(QInstaller::lcInstallerInstallLog)
          << "Cannot replace default resource with"
  ...
  foreach (const Range<qint64> &segment, existingResourceSegments) {
    ...
    resourceSegments.append(
        Range<qint64>::fromStartAndLength(output->pos(), segment.length()));
  ...
  QInstaller::appendInt64(output,
                          numComponents); // and 2 times number of components,
  QInstaller::appendInt64(
      output, numComponents); // one before and one after the components
  const qint64 compIndexEnd = output->pos();

  QInstaller::appendInt64Range(
      output, Range<qint64>::fromStartAndEnd(compIndexStart, compIndexEnd)
...
void PackageManagerCorePrivate::writeMaintenanceToolAppBundle(
  ...
  if (isInstaller() && QInstaller::isInBundle(sourceAppDirPath)) {
    ...
    op->setArguments(QStringList()
                     << (sourceAppDirPath + QLatin1String("/../PkgInfo"))
    ...
    op->setArguments(QStringList()
                     << (sourceAppDirPath + QLatin1String("/../Info.plist"))
    ...
    const QString before =
        ...
        QLatin1String("</string>");
    const QString after = QLatin1String("<string>") +
                          QFileInfo(maintenanceToolName()).baseName() +
                          QLatin1String("</string>");
    while (!in.atEnd())
    ...
    if (!op->performOperation()) {
      qCWarning(QInstaller::lcInstallerInstallLog)
          << "ERROR in Mkdir operation:" << op->errorString();
    }
    ...
    op = createOwnedOperation(QLatin1String("Mkdir"));
    op->setArguments(QStringList() << (QFileInfo(targetAppDirPath).path() +
    ...
    const QString icon =
        ...
        QLatin1String(".icns");
    op = createOwnedOperation(QLatin1String("Copy"));
    op->setArguments(QStringList() << (sourceAppDirPath +
    ...
    op = createOwnedOperation(QLatin1String("CopyDirectory"));
    op->setArguments(QStringList()
                     << (sourceAppDirPath + QLatin1String("/../Frameworks"))
                     << (targetAppDirPath + QLatin1String("/../Frameworks")));
    performOperationThreaded(op);

    op = createOwnedOperation(QLatin1String("CopyDirectory"));
    op->setArguments(QStringList()
                     << (sourceAppDirPath + QLatin1String("/../plugins"))
...
void PackageManagerCorePrivate::writeMaintenanceTool(
    OperationList performedOperations) {
  ...
  try {
    ...
    //   |--- remember to restart and that we need to replace the original
    //   binary
    //
    // 2 - if we do not have a replacement, try to open the binary data file as
    // input
    //   |--- try to read the binary layout
    //      |--- on error (see 2.1)
    //          |--- remember we might to append uncompressed resource data (see
    //          3)
    //          |--- set the installer or maintenance binary as input to take
    //          over binary data
    //          |--- in case we did not have a replacement, write out an new
    //          maintenance tool binary
    //              |--- remember that we need to replace the original binary
    //
    // 3 - open a new binary data file
    //   |--- try to write out the binary data based on the loaded input file
    //   (see 2)
    //      |--- on error (see 3.1)
    //          |--- if we wrote a new maintenance tool, take this as output -
    //          if not, write out a new
    //                  one and set it as output file, remember we did this
    //          |--- append the binary data based on the loaded input file (see
    //          2), make sure we force
    //                 uncompressing the resource section if we read from a
    //                 binary data file (see 4.1).
    //
    // 4 - force a deferred rename on the .dat file (see 4.1)
    // 5 - force a deferred rename on the maintenance file (see 5.1)

    // 2.1 - Error cases are: no data file (in fact we are the installer or an
    // old installation),
    //          could not find the data file magic cookie (unknown .dat file),
    //          failed to read binary layout (mostly likely the resource section
    //          or we couldn't seek inside the file)
    //
    // 3.1 - most likely the commit operation will fail
    // 4.1 - if 3 failed, this makes sure the .dat file will get removed and on
    // the next run all
    //          binary data is read from the maintenance tool, otherwise it get
    ...
    const QString installerBaseBinary =
        replaceVariables(m_installerBaseBinaryUnreplaced);
    if (!installerBaseBinary.isEmpty() &&
        QFileInfo::exists(installerBaseBinary)) {
      qCDebug(QInstaller::lcInstallerInstallLog)
          << "Got a replacement installer base binary:" << installerBaseBinary;
      if (QInstaller::isInBundle(installerBaseBinary)) {
        // In macOS the installerbase is a whole app bundle. We do not modify
        // the maintenancetool name in app bundle so that possible signing and
        // notarization will remain. Therefore, the actual maintenance tool name
        // might differ from the one defined in the settings.
        try {
          const QString maintenanceToolRenamedName =
              installerBaseBinary + QLatin1String(".new");
          qCDebug(QInstaller::lcInstallerInstallLog)
              << "Writing maintenance tool " << maintenanceToolRenamedName;
          QInstaller::copyDirectoryContents(installerBaseBinary,
                                            maintenanceToolRenamedName);
        ...
        try {
          ...
          qCDebug(QInstaller::lcInstallerInstallLog)
              << "Removed installer base binary" << installerBaseBinary
              << "after updating the maintenance tool.";
        } catch (const Error &error) {
          qCDebug(QInstaller::lcInstallerInstallLog)
              << "Cannot remove installer base binary" << installerBaseBinary
              << "after updating the maintenance tool:" << error.message();
        }
      } else {
        ...
        try {
          ...
          writeMaintenanceToolBinary(&replacementBinary,
                                     replacementBinary.size(), true);
          qCDebug(QInstaller::lcInstallerInstallLog)
              << "Wrote the binary with the new replacement.";
        ...
        if (!replacementBinary.remove()) {
          // Is there anything more sensible we can do with this error? I think
          // not. It's not serious enough for throwing / aborting the process.
          qCDebug(QInstaller::lcInstallerInstallLog)
              << "Cannot remove installer base binary" << installerBaseBinary
              << "after updating the maintenance tool:"
      ...
      m_installerBaseBinaryUnreplaced.clear();
               ...
               !QFileInfo::exists(installerBaseBinary)) {
      qCWarning(QInstaller::lcInstallerInstallLog)
          << "The current maintenance tool could not be updated."
          << installerBaseBinary
          << "does not exist. Please fix the \"setInstallerBaseBinary"
    ...
    try {
      if (isInstaller()) {
        if (QFile::exists(dataFile)) {
          qCWarning(QInstaller::lcInstallerInstallLog)
              << "Found binary data file" << dataFile
              << "but deliberately not used. Running as installer requires to "
                 "read the "
      ...
      layout =
          BinaryContent::binaryLayout(&input, BinaryContent::MagicCookieDat);
    } catch (const Error & /*error*/) {
      // We are only here when using installer
      QString binaryName = installerBinaryPath();
      // On Mac data is always in a separate file so that the binary can be
      // signed. On other platforms data is in separate file only after install
      // so that the maintenancetool sign does not break.
#ifdef Q_OS_MACOS
...
#else
        writeMaintenanceToolBinary(
            &tmp, layout.endOfBinaryContent - layout.binaryContentSize, true);
#endif
      }
    }

    performedOperations =
        sortOperationsBasedOnComponentDependencies(performedOperations);
    m_core->setValue(QLatin1String("installedOperationAreSorted"),
                     QLatin1String("true"));

    try {
      QFile file(generateTemporaryFileName());
      QInstaller::openForWrite(&file);
      writeMaintenanceToolBinaryData(&file, &input, performedOperations,
                                     layout);
      ...
      if (!newBinaryWritten) {
        newBinaryWritten = true;
        QFile tmp(isInstaller() ? installerBinaryPath()
                                : maintenanceToolName());
        QInstaller::openForRead(&tmp);
        BinaryLayout tmpLayout =
            BinaryContent::binaryLayout(&tmp, BinaryContent::MagicCookie);
        writeMaintenanceToolBinary(
            &tmp, tmpLayout.endOfBinaryContent - tmpLayout.binaryContentSize,
            false);
      }
    ...
    if (newBinaryWritten) {
      ...
      if (!aliasPath.isEmpty()) {
        // The new alias file is created after the maintenance too binary is
        // renamed, but we need to set the value before the variables get
        // written to disk.
        m_core->setValue(QLatin1String("CreatedMaintenanceToolAlias"),
                         aliasPath);
      }
    }
#endif
    ...
    QFile::rename(dataFile + QLatin1String(".new"),
                  targetDir() + QLatin1Char('/') + fi.baseName() +
                      QLatin1String(".dat"));

    const bool restart = !statusCanceledOrFailed() && m_needsHardRestart;
    qCDebug(QInstaller::lcInstallerInstallLog)
        << "Maintenance tool hard restart:" << (restart ? "true." : "false.");
...
void PackageManagerCorePrivate::writeOfflineBaseBinary() {
...
#ifndef Q_OS_MACOS
  BinaryLayout layout =
      BinaryContent::binaryLayout(&input, BinaryContent::MagicCookie);
  ...
  const QString offlineBinaryTempName =
      offlineBinaryName() + QLatin1String(".new");
  qCDebug(QInstaller::lcInstallerInstallLog)
      << "Writing offline base binary:" << offlineBinaryTempName;
  ProgressCoordinator::instance()->emitLabelAndDetailTextChanged(
      tr("Writing offline base binary."));
...

  {
    ...
    if (dummy.exists() && !dummy.remove()) {
      throw Error(tr("Cannot remove file \"%1\": %2")
                      .arg(dummy.fileName(), dummy.errorString()));
    }
    // Offline binary name might contain non-existing leading directories
    const QString offlineBinaryAbsolutePath =
        QFileInfo(offlineBinaryTempName).absolutePath();
    QDir dummyDir(offlineBinaryAbsolutePath);
    if (!dummyDir.exists() && !dummyDir.mkpath(offlineBinaryAbsolutePath)) {
      throw Error(
          tr("Cannot create directory \"%1\".").arg(dummyDir.absolutePath()));
    }
  }

  if (!out.copy(offlineBinaryTempName)) {
    throw Error(tr("Cannot write offline binary to \"%1\": %2")
                    .arg(offlineBinaryTempName, out.errorString()));
  }

  if (out.exists() && !out.remove()) {
    qCWarning(QInstaller::lcInstallerInstallLog)
        << tr("Cannot remove temporary file \"%1\": %2")
               .arg(out.fileName(), out.errorString());
  }
}

void PackageManagerCorePrivate::writeMaintenanceToolAlias(
    const QString &maintenanceToolName) {
#ifdef Q_OS_MACOS
  const QString aliasPath = maintenanceToolAliasPath();
  if (aliasPath.isEmpty())
    return;

  QString maintenanceToolBundle =
      QString::fromLatin1("%1/%2").arg(targetDir(), maintenanceToolName);
...
bool PackageManagerCorePrivate::runInstaller() {
  ...
  try {
    ...
    const QString target = QDir::cleanPath(
        targetDir().replace(QLatin1Char('\\'), QLatin1Char('/')));
    if (target.isEmpty())
      throw Error(tr("Variable 'TargetDir' not set."));

    if (!QDir(target).exists()) {
      const QString &pathToTarget =
          target.mid(0, target.lastIndexOf(QLatin1Char('/')));
      if (!QDir(pathToTarget).exists()) {
        Operation *pathToTargetOp =
            createOwnedOperation(QLatin1String("Mkdir"));
    ...
    ProgressCoordinator::instance()->emitLabelAndDetailTextChanged(
        tr("Preparing the installation..."));

    const QList<Component *> componentsToInstall =
        m_core->orderedComponentsToInstall();
    qCDebug(QInstaller::lcInstallerInstallLog)
        << "Install size:" << componentsToInstall.size() << "components";

    callBeginInstallation(componentsToInstall);
    stopProcessesForUpdates(componentsToInstall);

    if (m_dependsOnLocalInstallerBinary &&
        !KDUpdater::pathIsOnLocalDevice(qApp->applicationFilePath())) {
    ...
    const int downloadedArchivesCount =
        m_core->downloadNeededArchives(downloadPartProgressSize);

    // if there was no download we have the whole progress for installing
    // components
    if (!downloadedArchivesCount)
      componentsInstallPartProgressSize = double(1);

    // Force an update on the components xml as the install dir might have
    // changed.
    m_localPackageHub->setFileName(componentsXmlPath());
    // Clear the packages as we might install into an already existing
    // installation folder.
    m_localPackageHub->clearPackageInfos();
    // also update the application name, might be set from a script as well
    m_localPackageHub->setApplicationName(
        m_data
            .value(QLatin1String("ProductName"),
                   m_data.settings().applicationName())
            .toString());
    m_localPackageHub->setApplicationVersion(
        QLatin1String(QUOTE(IFW_REPOSITORY_FORMAT_VERSION)));

    const int progressOperationCount =
        countProgressOperations(componentsToInstall)
    ...
    double progressOperationSize =
        componentsInstallPartProgressSize / progressOperationCount;

    // Now install the requested components
    unpackAndInstallComponents(componentsToInstall, progressOperationSize,
                               adminRightsGained);

    if (m_core->isOfflineOnly() &&
        PackageManagerCore::createLocalRepositoryFromBinary()) {
      ...
      ProgressCoordinator::instance()->emitLabelAndDetailTextChanged(
          tr("Creating local repository"));

      Operation *createRepo =
          createOwnedOperation(QLatin1String("CreateLocalRepository"));
      if (createRepo) {
        QString binaryFile = QCoreApplication::applicationFilePath();
#ifdef Q_OS_MACOS
        // The installer binary on macOS does not contain the binary content,
        // it's put into the resources folder as separate file. Adjust the
        // actual binary path. No error checking here since we will fail later
        // while reading the binary content.
        ...
        createRepo->setArguments(QStringList()
                                 << binaryFile
        ...
        if (success) {
          ...
          repos.insert(Repository(
              QUrl::fromUserInput(
                  createRepo->value(QLatin1String("local-repo")).toString()),
              true));
          m_data.settings().setDefaultRepositories(repos);
          ...
          MessageBoxHandler::critical(
              MessageBoxHandler::currentBestSuitParent(),
              QLatin1String("installationError"), tr("Error"),
              createRepo->errorString());
    ...
    const int progress =
        ProgressCoordinator::instance()->progressInPercentage();
    // usually this should be only the reserved one from the beginning
    if (progress < 100)
      ProgressCoordinator::instance()->addManualPercentagePoints(100 -
                                                                 progress);
    ...
    if (m_core->status() != PackageManagerCore::Canceled) {
      setStatus(PackageManagerCore::Failure);
      MessageBoxHandler::critical(MessageBoxHandler::currentBestSuitParent(),
                                  QLatin1String("installationError"),
...
bool PackageManagerCorePrivate::runPackageUpdater() {
  ...
  try {
    ...
    if (m_dependsOnLocalInstallerBinary &&
        !KDUpdater::pathIsOnLocalDevice(qApp->applicationFilePath())) {
      throw Error(tr(
          "It is not possible to run that operation from a network location"));
    }
    ...
    // next loop will save the needed operations in reverse order for
    // uninstallation
    OperationList performedOperationsOld = m_performedOperationsOld;
    if (m_core->value(QLatin1String("installedOperationAreSorted")) !=
        QLatin1String("true"))
      performedOperationsOld =
          sortOperationsBasedOnComponentDependencies(m_performedOperationsOld);

    // build a list of undo operations based on the checked state of the
    // component
    foreach (Operation *operation, performedOperationsOld) {
      ...
      if (!component)
        component =
            m_core->componentByName(PackageManagerCore::checkableName(name));
      if (component)
        componentsByName.insert(name, component);

      if (isUpdater()) {
        // We found the component, the component is not scheduled for update,
        // the dependency solver did not add the component as install dependency
        // and there is no replacement, keep it.
        if ((component && !component->updateRequested() &&
        ...
        // There is a replacement, but the replacement is not scheduled for
        // update, keep it as well.
        if (m_componentsToReplaceUpdaterMode.contains(name) &&
            !m_installerCalculator->resolvedComponents().contains(
                m_componentsToReplaceUpdaterMode.value(name).first)) {
          nonRevertedOperations.append(operation);
          continue;
        }
      } else if (isPackageManager()) {
        // We found the component, the component is still checked and the
        // dependency solver did not add the component as install dependency,
        // keep it.
        if (component &&
        ...
        if (m_componentsToReplaceAllMode.contains(name) &&
            !m_componentsToReplaceAllMode.value(name)
                 .first->isSelectedForInstallation()) {
          nonRevertedOperations.append(operation);
          continue;
        }
      } else {
      ...
      // Filter out the create target dir undo operation, it's only needed for
      // full uninstall. Note: We filter for unnamed operations as well, since
      // old installations had the remove target
      //  dir operation without the "uninstall-only", which will result in a
      //  complete uninstallation during an update for the maintenance tool.
    ...
    // we did not request admin rights till we found out that a component/ undo
    // needs admin rights
    ...
    if (undoOperations.count() > 0) {
      ProgressCoordinator::instance()->emitLabelAndDetailTextChanged(
          tr("Removing deselected components..."));
      runUndoOperations(undoOperations, undoOperationProgressSize,
                        adminRightsGained, true);
    }
    m_performedOperationsOld =
        nonRevertedOperations; // these are all operations left: those not
                               // reverted

    const double progressOperationCount =
        countProgressOperations(componentsToInstall);
...
bool PackageManagerCorePrivate::runUninstaller() {
  ...
  try {
    ...
    if (!adminRightsGained) {
      foreach (Operation *op, m_performedOperationsOld) {
        ...
        if (updateAdminRights)
          break; // an operation needs elevation to be able to perform their
                 // undo
      }
    }
    ...
    const double undoOperationProgressSize =
        double(1) / double(uninstallOperationCount);

    runUndoOperations(undoOperations, undoOperationProgressSize,
                      adminRightsGained, false);
    // No operation delete here, as all old undo operations are deleted in the
    // destructor.
  ...
  ProgressCoordinator::instance()->emitLabelAndDetailTextChanged(
      QString::fromLatin1("\n%1").arg(
          success ? tr("Removal completed successfully.")
                  : tr("Removal aborted.")));
...
bool PackageManagerCorePrivate::runOfflineGenerator() {
  ...
  try {
    ...
    ProgressCoordinator::instance()->addManualPercentagePoints(1);
    ProgressCoordinator::instance()->emitLabelAndDetailTextChanged(
        tr("Preparing offline generation..."));

    const QList<Component *> componentsToInclude =
        m_core->orderedComponentsToInstall();
    qCDebug(QInstaller::lcInstallerInstallLog)
        << "Included components:" << componentsToInclude.size();

    // Give full part progress size as this is the most time consuming step
    m_core->downloadNeededArchives(double(1));

    const QString installerBaseReplacement =
        replaceVariables(m_offlineBaseBinaryUnreplaced);
    if (!installerBaseReplacement.isEmpty() &&
        QFileInfo::exists(installerBaseReplacement)) {
      qCDebug(QInstaller::lcInstallerInstallLog)
          << "Got a replacement installer base binary:"
          << offlineBinaryTempName;

      if (!QFile::copy(installerBaseReplacement, offlineBinaryTempName)) {
        qCWarning(QInstaller::lcInstallerInstallLog)
            << QString::fromLatin1("Cannot copy "
                                   "replacemement binary to temporary location "
                                   "\"%1\" from \"%2\".")
    ...
    ProgressCoordinator::instance()->emitLabelAndDetailTextChanged(
        tr("Preparing installer configuration..."));

    // Create copy of internal config file and data, remove repository related
    // elements
    QInstaller::trimmedCopyConfigData(
        m_data.settingsFilePath(), tempSettingsFilePath,
    ...
    ProgressCoordinator::instance()->emitLabelAndDetailTextChanged(
        tr("Creating the installer..."));

    QString errorMessage;
    if (QInstallerTools::createBinary(args, errorMessage) == EXIT_FAILURE) {
      throw Error(
          tr("Failed to create offline installer. %1").arg(errorMessage));
    } else {
      setStatus(PackageManagerCore::Success);
    }
  ...
  if (tempSettingsDir.exists() && !tempSettingsDir.removeRecursively()) {
    qCWarning(QInstaller::lcInstallerInstallLog)
        << tr("Cannot remove temporary directory \"%1\".")
  ...
  ProgressCoordinator::instance()->emitLabelAndDetailTextChanged(
      QString::fromLatin1("\n%1").arg(
          success ? tr("Offline generation completed successfully.")
                  : tr("Offline generation aborted!")));

  emit offlineGenerationFinished();
  return success;
}

void PackageManagerCorePrivate::unpackComponents(
    const QList<Component *> &components, double progressOperationSize,
  ...
  for (auto *component : components) {
    ...
    for (auto &op : operations) {
      ...
      // any op requesting execution as admin means all extracts are done as
      // admin.
      if (!adminRightsGained && !becameAdmin &&
          op->value(QLatin1String("admin")).toBool())
        becameAdmin = m_core->gainAdminRights();
    }
  }

  // 2. Calculate proportional progress sizes
  const double totalOperationsSize =
      progressOperationSize * unpackOperations.size();
  for (auto *op : unpackOperations) {
    const double ratio =
        static_cast<double>(op->sizeHint()) / totalOperationsSizeHint;
    const double proportionalProgressOperationSize =
        totalOperationsSize * ratio;
  ...
  connect(m_core, &PackageManagerCore::installationInterrupted, &runner,
          &ConcurrentOperationRunner::cancel);

  connect(&runner, &ConcurrentOperationRunner::progressChanged,
  ...
  for (auto &operation : backupOperations) {
    ...
    if (!backupResults.value(operation) ||
        operation->error() != Operation::NoError) {
      // For Extract, backup stops only on read errors. That means the perform
      // step will also fail later on, which handles the user selection on what
      // to do with the error.
      qCWarning(QInstaller::lcInstallerInstallLog)
          << QString::fromLatin1("Backup of operation "
                                 "\"%1\" with arguments \"%2\" failed: %3")
                 .arg(operation->name(),
                      operation->arguments().join(QLatin1String("; ")),
  ...
  // TODO: Do we need to rollback backups too? For Extract op this works
  // without, as the backup files are not used in Undo step.
  if (statusCanceledOrFailed())
    throw Error(tr("Installation canceled by user"));

  // 4. Perform operations
  std::sort(unpackOperations.begin(), unpackOperations.end(),
  ...
  for (auto &operation : performedOperations) {
    ...
    while (!ok && !ignoreError &&
           m_core->status() != PackageManagerCore::Canceled) {
      qCDebug(QInstaller::lcInstallerInstallLog)
          << QString::fromLatin1("Operation \"%1\" with arguments "
                                 "\"%2\" failed: %3")
                 .arg(operation->name(),
                      operation->arguments().join(QLatin1String("; ")),
                      operation->errorString());
      ...
      const QMessageBox::StandardButton button = MessageBoxHandler::warning(
          ...
          tr("Error during installation process (%1):\n%2")
              .arg(component, operation->errorString()),
          QMessageBox::Retry | QMessageBox::Ignore | QMessageBox::Cancel,
          QMessageBox::Cancel);
...
void PackageManagerCorePrivate::installComponent(Component *component,
                                                 double progressOperationSize,
                                                 bool adminRightsGained) {
  ...
  // show only components which do something, MinimumProgress is only for
  // progress calculation safeness
  bool showDetailsLog = false;
  if (opCount > 1 || (opCount == 1 && operations.at(0)->name() !=
                                          QLatin1String("MinimumProgress"))) {
    ProgressCoordinator::instance()->emitLabelAndDetailTextChanged(
        QLatin1Char('\n') +
  ...
  foreach (Operation *operation, operations) {
    ...
    if (!adminRightsGained &&
      ...
      becameAdmin = m_core->gainAdminRights();
      qCDebug(QInstaller::lcInstallerInstallLog)
          << operation->name() << "as admin:" << becameAdmin;
    }
    ...
    while (!ok && !ignoreError &&
      ...
      const QMessageBox::StandardButton button = MessageBoxHandler::warning(
          ...
          QLatin1String("installationErrorWithCancel"), tr("Installer Error"),
          tr("Error during installation process (%1):\n%2")
              .arg(component->name(), operation->errorString()),
          QMessageBox::Retry | QMessageBox::Ignore | QMessageBox::Cancel,
          QMessageBox::Cancel);
    ...
    if (ok || operation->error() > Operation::InvalidArguments) {
      // Remember that the operation was performed, that allows us to undo it if
      // a following operation fails or if this operation failed but still needs
  ...
  if (!m_core->isCommandLineInstance()) {
    ...
    else if ((component->value(scForcedUpdate, scFalse) == scTrue) &&
             isUpdater())
      m_needsHardRestart = true;
  }

  registerPathsForUninstallation(component->pathsForUninstallation(),
                                 component->name());

  if (!component->stopProcessForUpdateRequests().isEmpty()) {
    Operation *stopProcessForUpdatesOp =
        KDUpdater::UpdateOperationFactory::instance().create(
            QLatin1String("FakeStopProcessForUpdate"), m_core);
    const QStringList arguments(
        component->stopProcessForUpdateRequests().join(QLatin1String(",")));
    stopProcessForUpdatesOp->setArguments(arguments);
    addPerformed(stopProcessForUpdatesOp);
    stopProcessForUpdatesOp->setValue(QLatin1String("component"),
                                      component->name());
  }

  // now mark the component as installed
  m_localPackageHub->addPackage(
      component->name(), component->value(scVersion),
      ...
      component->value(scSortingPriority).toInt(), component->dependencies(),
      component->autoDependencies(), component->forcedInstallation(),
      component->isVirtual(),
      component->value(scUncompressedSize).toULongLong(),
      component->value(scInheritVersion), component->isCheckable(),
      component->isExpandedByDefault(), component->value(scContentSha1));
...
bool PackageManagerCorePrivate::runningProcessesFound() {
  ...
  const QString performModeWarning =
      m_completeUninstall ? QLatin1String("Unable to remove components.")
  ...
  if (!runningProcesses.isEmpty()) {
    qCWarning(QInstaller::lcInstallerInstallLog).noquote().nospace()
        << performModeWarning
        << " Please stop these processes: " << runningProcesses
        << " and try again.";
    return true;
  }
  return false;
...
void PackageManagerCorePrivate::setComponentSelection(const QString &id,
                                                      Qt::CheckState state) {
  ComponentModel *model = m_core->isUpdater() ? m_core->updaterComponentModel()
...
void PackageManagerCorePrivate::deleteMaintenanceTool() {
  ...
  installDir.remove(m_data.settings().maintenanceToolName() +
                    QLatin1String(".dat"));
  ...
  installDir.remove(m_core->value(QLatin1String("LogFileName"),
                                  QLatin1String("InstallationLog.txt")));
#ifdef Q_OS_WIN
  // Since Windows does not support that the maintenance tool deletes itself we
  // have to go with a rather dirty hack. What we do is to create a batchfile
  // that will try to remove the maintenance tool once per second. Then we start
  // that batchfile detached, finished our job and close ourselves. Once that's
  // done the batchfile will succeed in deleting our uninstall.exe and, if the
  // installation directory was created but us and if it's empty after the
  // uninstall, deletes the installation-directory.
  const QString batchfile = QDir::toNativeSeparators(
      QFileInfo(QDir::tempPath(), QLatin1String("uninstall.vbs"))
          .absoluteFilePath());
  QFile f(batchfile);
  if (!f.open(QIODevice::WriteOnly | QIODevice::Text))
    throw Error(tr("Cannot prepare removal"));

  const bool removeTargetDir =
      QVariant(m_core->value(scRemoveTargetDir)).toBool();
  QTextStream batch(&f);
  ...
  arguments << QDir::toNativeSeparators(
      QFileInfo(installerBinaryPath()).absoluteFilePath());
  arguments << targetDir();

  if (!QProcessWrapper::startDetached(QLatin1String("cscript"), arguments,
                                      QDir::rootPath()))
  ...
  if (QInstaller::isInBundle(installerBinaryPath())) {
    ...
    removeDirectoryThreaded(
        QFileInfo(installerBinaryPath() + cdUp).absoluteFilePath());
...
#endif
  {
    ...
    QFile::remove(QFileInfo(installerBinaryPath()).absolutePath() +
                  QLatin1String("/") + configurationFileName());
  }
}
...
#ifdef Q_OS_MACOS
  const QString maintenanceToolAlias =
      m_core->value(QLatin1String("CreatedMaintenanceToolAlias"));
  QFile aliasFile(maintenanceToolAlias);
  if (!maintenanceToolAlias.isEmpty() && !aliasFile.remove()) {
    // Not fatal
    qWarning(lcInstallerInstallLog)
        << "Cannot remove alias file" << maintenanceToolAlias
        << "for maintenance tool:" << aliasFile.errorString();
  }
#endif
}
...
void PackageManagerCorePrivate::registerMaintenanceTool() {
  ...
  settings.setValue(QLatin1String("DisplayVersion"),
                    m_data.value(QLatin1String("ProductVersion")));
  const QString maintenanceTool =
      QDir::toNativeSeparators(maintenanceToolName());
  settings.setValue(QLatin1String("DisplayIcon"), maintenanceTool);
  settings.setValue(scPublisher, m_data.value(scPublisher));
  settings.setValue(QLatin1String("UrlInfoAbout"),
                    m_data.value(QLatin1String("Url")));
  settings.setValue(QLatin1String("Comments"), m_data.value(scTitle));
  settings.setValue(QLatin1String("InstallDate"),
                    QDateTime::currentDateTime().toString());
  settings.setValue(QLatin1String("InstallLocation"),
                    QDir::toNativeSeparators(targetDir()));
  settings.setValue(QLatin1String("UninstallString"), quoted(maintenanceTool));
  settings.setValue(
      QLatin1String("ModifyPath"),
      QString(quoted(maintenanceTool) + QLatin1String(" --manage-packages")));
  ...
  // Windows can only handle 32bit REG_DWORD (max. recordable installation size
  // is 4TiB)
  const quint64 limit =
      std::numeric_limits<quint32>::max(); // maximum 32 bit value
  if (estimatedSizeKB <= limit)
    settings.setValue(QLatin1String("EstimatedSize"),
                      static_cast<quint32>(estimatedSizeKB));
...
void PackageManagerCorePrivate::runUndoOperations(
    const OperationList &undoOperations, double progressSize,
  ...
  try {
    ...
    foreach (Operation *undoOperation, undoOperations) {
      ...
      bool becameAdmin = false;
      if (!adminRightsGained &&
          undoOperation->value(QLatin1String("admin")).toBool())
        becameAdmin = m_core->gainAdminRights();

      connectOperationToInstaller(undoOperation, progressSize);
      qCDebug(QInstaller::lcInstallerInstallLog)
          << "undo operation=" << undoOperation->name();

      bool ignoreError = false;
      bool ok = performOperationThreaded(undoOperation, Operation::Undo);

      const QString componentName =
          undoOperation->value(QLatin1String("component")).toString();
      ...
      if (!componentName.isEmpty()) {
        while (!ok && !ignoreError &&
          ...
          const QMessageBox::StandardButton button = MessageBoxHandler::warning(
              MessageBoxHandler::currentBestSuitParent(),
              QLatin1String("installationErrorWithIgnore"),
              tr("Installer Error"),
              tr("Error during removal process:\n%1")
                  .arg(undoOperation->errorString()),
        ...
        Component *component = m_core->componentByName(
            PackageManagerCore::checkableName(componentName));
        if (!component)
      ...
      ProgressCoordinator::instance()->emitAdditionalProgressStatus(
          tr("%1 of %2 operations rolled back.")
              .arg(QString::number(rolledBackOperations),
                   QString::number(operationsCount)));
...
/*!
    Returns a hash containing the installed package name and it's associated
   package information. If the application is running in installer mode or the
   local components file could not be parsed, the hash is empty.
*/
LocalPackagesMap PackageManagerCorePrivate::localInstalledPackages() {
  ...
  if (m_localPackageHub->error() != LocalPackageHub::NoError) {
    setStatus(PackageManagerCore::Failure,
              tr("Failure to read packages from %1.").arg(componentsXmlPath()));
  }
...
bool PackageManagerCorePrivate::fetchMetaInformationFromRepositories(
  ...
  try {
    ...
    setStatus(PackageManagerCore::Failure,
              tr("Cannot retrieve meta information: %1").arg(error.message()));
...
bool PackageManagerCorePrivate::addUpdateResourcesFromRepositories(
  ...
  foreach (const Metadata *data, metadata) {
    ...
    if (data->repository().isCompressed())
      m_compressedPackageSources.insert(
          PackageSource(QUrl::fromLocalFile(data->path()), 2));
    else
      m_packageSources.insert(
          PackageSource(QUrl::fromLocalFile(data->path()), 0));

    ProductKeyCheck::instance()->addPackagesFromXml(
        data->path() + QLatin1String("/Updates.xml"));
  }
  if ((compressedRepository && m_compressedPackageSources.count() == 0) ||
      (!compressedRepository && m_packageSources.count() == 0)) {
    setStatus(PackageManagerCore::Failure,
              tr("Cannot find any update source information."));
    return false;
  }
...
void PackageManagerCorePrivate::connectOperationCallMethodRequest(
    Operation *const operation) {
  QObject *const operationObject = dynamic_cast<QObject *>(operation);
  if (operationObject != nullptr) {
    const QMetaObject *const mo = operationObject->metaObject();
    if (mo->indexOfSignal(QMetaObject::normalizedSignature(
            "requestBlockingExecution(QString)")) > -1) {
      connect(operationObject, SIGNAL(requestBlockingExecution(QString)), this,
              SLOT(handleMethodInvocationRequest(QString)),
...
OperationList
PackageManagerCorePrivate::sortOperationsBasedOnComponentDependencies(
  ...
  foreach (const Component *node,
           m_core->components(PackageManagerCore::ComponentType::All)) {
    componentGraph.addNode(node->name());
    componentGraph.addEdges(node->name(),
                            m_core->parseNames(node->dependencies()));
  }

  const QStringList resolvedComponents = componentGraph.sort();
  if (componentGraph.hasCycle()) {
    throw Error(
        tr("Dependency cycle between components \"%1\" and \"%2\" detected.")
...
/*
    Adds the \a path for deletetion. Unlike files for delayed deletion, which
   are deleted on the start of next installer run, these paths are deleted on
   exit.
*/
...
void PackageManagerCorePrivate::unpackAndInstallComponents(
    const QList<Component *> &components, const double progressOperationSize,
    const bool adminRightsGained) {
  ...
  foreach (Component *component, components) {
    ...
    ++installedComponents;
    ProgressCoordinator::instance()->emitAdditionalProgressStatus(
        tr("%1 of %2 components installed.")
            .arg(QString::number(installedComponents),
                 QString::number(componentsToInstallCount)));
  }
  ProgressCoordinator::instance()->emitAdditionalProgressStatus(
      tr("All components installed."));
}
...
void PackageManagerCorePrivate::findExecutablesRecursive(
    const QString &path, const QStringList &excludeFiles, QStringList *result) {
  QDirIterator it(path,
                  QDir::NoDotAndDotDot | QDir::Executable | QDir::Files |
                      QDir::System,
                  QDirIterator::Subdirectories);
...
QStringList PackageManagerCorePrivate::runningInstallerProcesses(
    const QStringList &excludeFiles) {
  QStringList resultFiles;
  findExecutablesRecursive(QCoreApplication::applicationDirPath(), excludeFiles,
                           &resultFiles);
  ...
  for (int i = priorities.length() - 1; i >= 0; --i) {
    ...
    for (QString licenseName : licenseNames) {
      if (m_autoAcceptLicenses ||
        ...
        qCDebug(QInstaller::lcInstallerInstallLog)
            << "License" << licenseName << "accepted by user.";
      } else {
        qCDebug(QInstaller::lcInstallerInstallLog)
            << "License" << licenseName << "not accepted by user. Aborting.";
        return false;
      }
    }
  }
...
bool PackageManagerCorePrivate::askUserAcceptLicense(
    const QString &name, const QString &content) const {
  qCDebug(QInstaller::lcInstallerInstallLog)
      << "You must accept "
  ...
  forever {
    const QString input =
        m_core->readConsoleLine(QLatin1String("Accept|Reject|Show"));

    if (QString::compare(input, QLatin1String("Accept"), Qt::CaseInsensitive) ==
            0 ||
        QString::compare(input, QLatin1String("A"), Qt::CaseInsensitive) == 0) {
      return true;
    } else if (QString::compare(input, QLatin1String("Reject"),
                                Qt::CaseInsensitive) == 0 ||
               QString::compare(input, QLatin1String("R"),
                                Qt::CaseInsensitive) == 0) {
      return false;
    } else if (QString::compare(input, QLatin1String("Show"),
                                Qt::CaseInsensitive) == 0 ||
               QString::compare(input, QLatin1String("S"),
                                Qt::CaseInsensitive) == 0) {
...
bool PackageManagerCorePrivate::askUserConfirmCommand() const {
  ...
  forever {
    ...
    if (QString::compare(input, QLatin1String("Yes"), Qt::CaseInsensitive) ==
            0 ||
        QString::compare(input, QLatin1String("Y"), Qt::CaseInsensitive) == 0) {
      return true;
    } else if (QString::compare(input, QLatin1String("No"),
                                Qt::CaseInsensitive) == 0 ||
               QString::compare(input, QLatin1String("N"),
                                Qt::CaseInsensitive) == 0) {
      return false;
    } else {
      qCDebug(QInstaller::lcInstallerInstallLog) << "Unknown answer:" << input;
...
bool PackageManagerCorePrivate::packageNeedsUpdate(
    const LocalPackage &localPackage, const Package *update) const {
  ...
  for (auto &componentName : m_pendingUnstableComponents.keys()) {
    ...
    if (!component) {
      qCWarning(lcInstallerInstallLog) << "Failure while marking component "
                                          "unstable. No such component exists:"
                                       << componentName;
      continue;
    }
...
void PackageManagerCorePrivate::createAutoDependencyHash(
    const QString &component, const QString &oldDependencies,
    const QString &newDependencies) {
  // User might have changed autodependencies with setValue. Remove the old
  // values.
  const QStringList oldDependencyList =
      oldDependencies.split(QInstaller::commaRegExp(), Qt::SkipEmptyParts);
  ...
  const QStringList newDependencyList =
      newDependencies.split(QInstaller::commaRegExp(), Qt::SkipEmptyParts);
...
void PackageManagerCorePrivate::createLocalDependencyHash(
    const QString &component, const QString &dependencies) {
  const QStringList localDependencies =
      dependencies.split(QInstaller::commaRegExp(), Qt::SkipEmptyParts);
