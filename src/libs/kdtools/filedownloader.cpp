...
/*!
    ...
    \brief The FileDownloader class is the base class for file downloaders used
   in KDUpdater.

    File downloaders are used by the KDUpdater::Update class to download update
   files. Each subclass of FileDownloader can download files from a specific
   category of sources (such as \c local, \c ftp, \c http).

    This is an internal class, not a part of the public API. Currently we have
   the following subclasses of FileDownloader: \list \li HttpDownloader to
   download files over FTP, HTTP, or HTTPS if Qt is built with SSL. \li
   LocalFileDownloader to copy files from the local file system. \li
   ResourceFileDownloader to download resource files. \endlist
*/
...
/*!
    \fn KDUpdater::FileDownloader::authenticatorChanged(const QAuthenticator
   &authenticator) This signal is emitted when the authenticator changes to \a
   authenticator.
*/
...
/*!
    \fn KDUpdater::FileDownloader::downloadProgress(qint64 bytesReceived, qint64
   bytesToReceive) This signal is emitted with the download progress as the
   number of received bytes, \a bytesReceived, and the total size of the file to
   download, \a bytesToReceive.
*/

/*!
    \fn KDUpdater::FileDownloader::downloadSpeed(qint64 bytesPerSecond)
    This signal is emitted with the download speed in bytes per second as \a
   bytesPerSecond.
*/
...
/*!
    ...
    This signal is emitted with textual representation of the current download
   \a status in the following format: "100 MiB of 150 MiB - (DAYS) (HOURS)
...
/*!
    ...
    This signal is emitted when downloading a file is aborted with \a
   errorMessage.
*/
...
/*!
    \fn KDUpdater::FileDownloader::setDownloadedFileName(const QString &name) =
   0 Sets the file name of the downloaded file to \a name.
*/

struct KDUpdater::FileDownloader::Private {
  Private()
      : m_hash(QCryptographicHash::Sha1), m_assumedSha1Sum(""),
        autoRemove(true), followRedirect(false), m_speedTimerInterval(100),
        m_downloadDeadlineTimerInterval(30000), m_downloadPaused(false),
        m_downloadResumed(false), m_bytesReceived(0), m_bytesToReceive(0),
        m_bytesBeforeResume(0), m_totalBytesBeforeResume(0),
        m_currentSpeedBin(0), m_sampleIndex(0), m_downloadSpeed(0),
        m_factory(0), m_ignoreSslErrors(false) {
    memset(m_samples, 0, sizeof(m_samples));
  }

  ~Private() { delete m_factory; }
...
*/
KDUpdater::FileDownloader::FileDownloader(const QString &scheme,
                                          QObject *parent)
...
*/
KDUpdater::FileDownloader::~FileDownloader() { delete d; }

void KDUpdater::FileDownloader::setUrl(const QUrl &url) { d->url = url; }

QUrl KDUpdater::FileDownloader::url() const { return d->url; }
...
*/
QString FileDownloader::errorString() const { return d->errorString; }

/*!
    Sets the human readable description of the last error that occurred to \a
   error. Emits the downloadStatus() and downloadAborted() signals.
*/
...
/*!
    ...
    If an assumed SHA-1 checksum is set and the actual calculated checksum does
   not match it, sets the status to \c error. If no SHA-1 is assumed, no check
   is performed, and status is set to \c success.
...
QString KDUpdater::FileDownloader::scheme() const { return d->scheme; }
...
*/
void KDUpdater::FileDownloader::updateBytesDownloadedBeforeResume(
    qint64 bytes) {
...
/*!
    Sets the file download progress to the number of received bytes, \a
   bytesReceived, and the number of total bytes to receive, \a bytesToReceive.
*/
void KDUpdater::FileDownloader::setProgress(qint64 bytesReceived,
...
/*!
    Calculates the download speed in bytes per second and emits the
...
void KDUpdater::FileDownloader::emitDownloadSpeed() {
  ...
  d->m_downloadSpeed *=
      1000.0 / d->m_speedTimerInterval; // rescale to bytes/second

  emit downloadSpeed(d->m_downloadSpeed);
}

/*!
    Builds a textual representation of the download status in the following
   format: "100 MiB of 150 MiB - (DAYS) (HOURS) (MINUTES) (SECONDS) remaining".
...
void KDUpdater::FileDownloader::emitDownloadStatus() {
  ...
  if (d->m_bytesToReceive > 0) {
    ...
    // remove the unit from the bytesReceived value if bytesToReceive has the
    // same
    const QString tmp =
        bytesToReceive.mid(bytesToReceive.indexOf(QLatin1Char(' ')));
  ...
  status += QLatin1Char(' ') +
            tr("(%1/sec)").arg(humanReadableSize(d->m_downloadSpeed));
  if (d->m_bytesToReceive > 0 && d->m_downloadSpeed > 0) {
    const qint64 time =
        (d->m_bytesToReceive - d->m_bytesReceived) / d->m_downloadSpeed;
...
void KDUpdater::FileDownloader::emitEstimatedDownloadTime() {
  ...
  emit estimatedDownloadTime((d->m_bytesToReceive - d->m_bytesReceived) /
                             d->m_downloadSpeed);
}
...
/*!
    Adds the \a length of characters of \a data to the cryptographic hash of the
   downloaded file.
*/
...
*/
void KDUpdater::FileDownloader::resetCheckSumData() { d->m_hash.reset(); }

/*!
    Returns a copy of the proxy factory that this FileDownloader object is using
   to determine the proxies to be used for requests.
*/
...
/*!
    Sets the proxy factory for this class to be \a factory. A proxy factory is
   used to determine a more specific list of proxies to be used for a given
   request, instead of trying to use the same proxy value for all requests. This
   might only be of use for HTTP or FTP requests.
*/
void KDUpdater::FileDownloader::setProxyFactory(
    FileDownloaderProxyFactory *factory) {
  delete d->m_factory;
  d->m_factory = factory;
}

/*!
    Returns a copy of the authenticator that this FileDownloader object is using
   to set the username and password for a download request.
*/
...
/*!
    Sets the authenticator object for this class to be \a authenticator. An
   authenticator is used to pass on the required authentication information.
   This might only be of use for HTTP or FTP requests. Emits the authenticator
   changed signal with the new authenticator in use.
*/
void KDUpdater::FileDownloader::setAuthenticator(
...
*/
qint64 FileDownloader::getBytesReceived() const { return d->m_bytesReceived; }
...
/*!
    ...
    sometimes in parallel to other file downloaders. If copying a local file
   takes a long time, it will make the other downloads hang. Therefore, a timer
   is used and one block of data is copied per unit time, even though
   QFile::copy() does the task of copying local files from one place to another.
*/

struct KDUpdater::LocalFileDownloader::Private {
  Private() : source(0), destination(0), downloaded(false), timerId(-1) {}
...
KDUpdater::LocalFileDownloader::LocalFileDownloader(QObject *parent)
    : KDUpdater::FileDownloader(QLatin1String("file"), parent), d(new Private) {
}
...
void KDUpdater::LocalFileDownloader::doDownload() {
  ...
  if (!d->source->open(QFile::ReadOnly)) {
    setDownloadAborted(
        tr("Cannot open file \"%1\" for reading: %2")
            .arg(QFileInfo(localFile).fileName(), d->source->errorString()));
    onError();
    return;
  }
...
*/
void KDUpdater::LocalFileDownloader::setDownloadedFileName(
    const QString &name) {
  d->destFileName = name;
}
...
/*!
    Clones the local file downloader and assigns it the parent \a parent.
   Returns the new local file downloader.
*/
KDUpdater::LocalFileDownloader *
KDUpdater::LocalFileDownloader::clone(QObject *parent) const {
...
void KDUpdater::LocalFileDownloader::timerEvent(QTimerEvent *event) {
  if (event->timerId() == d->timerId) {
    ...
    while (toWrite > 0) {
      const qint64 numWritten = d->destination->write(
          buffer.constData() + numRead - toWrite, toWrite);
      if (numWritten < 0) {
        killTimer(d->timerId);
        d->timerId = -1;
        setDownloadAborted(
            tr("Writing to file \"%1\" failed: %2")
                .arg(QDir::toNativeSeparators(d->destination->fileName()),
                     d->destination->errorString()));
        onError();
        return;
      }
...
/*!
    ...
    \brief The ResourceFileDownloader class can be used to download resource
   files.
*/
struct KDUpdater::ResourceFileDownloader::Private {
  Private() : timerId(-1), downloaded(false) {}
...
*/
KDUpdater::ResourceFileDownloader::~ResourceFileDownloader() { delete d; }
...
*/
void KDUpdater::ResourceFileDownloader::setDownloadedFileName(
    const QString & /*name*/) {
  // Not supported!
}

/*!
    Clones the resource file downloader and assigns it the parent \a parent.
   Returns the new resource file downloader.
*/
KDUpdater::ResourceFileDownloader *
KDUpdater::ResourceFileDownloader::clone(QObject *parent) const {
...
void KDUpdater::ResourceFileDownloader::timerEvent(QTimerEvent *event) {
  if (event->timerId() == d->timerId) {
    if (!d->destFile.isOpen()) {
      ...
      setDownloadAborted(
          tr("Cannot read resource file \"%1\": %2")
              .arg(downloadedFileName(), d->destFile.errorString()));
      onError();
      return;
    }
    ...
    if (numRead > 0) {
      ...
      emit downloadProgress(
          calcProgress(d->destFile.pos(), d->destFile.size()));
      return;
    }
...
/*!
    ...
    \brief The HttpDownloader class is used to download files over FTP, HTTP, or
   HTTPS.
...
struct KDUpdater::HttpDownloader::Private {
  explicit Private(HttpDownloader *qq)
      : q(qq), http(0), destination(0), downloaded(false), aborted(false),
  ...
  void shutDown(bool closeDestination = true) {
    if (http) {
      disconnect(http, &QNetworkReply::finished, q,
                 &HttpDownloader::httpReqFinished);
      disconnect(http, &QNetworkReply::downloadProgress, q,
                 &HttpDownloader::httpReadProgress);
      disconnect(http, &QNetworkReply::errorOccurred, q,
                 &HttpDownloader::httpError);
      http->deleteLater();
    }
    http = 0;
    if (closeDestination) {
...
KDUpdater::HttpDownloader::HttpDownloader(QObject *parent)
    : KDUpdater::FileDownloader(QLatin1String("http"), parent),
      d(new Private(this)) {
#ifndef QT_NO_SSL
  connect(&d->manager, &QNetworkAccessManager::sslErrors, this,
          &HttpDownloader::onSslErrors);
#endif
  connect(&d->manager, &QNetworkAccessManager::authenticationRequired, this,
          &HttpDownloader::onAuthenticationRequired);
  connect(QNetworkInformation::instance(),
          &QNetworkInformation::reachabilityChanged, this,
          &HttpDownloader::onNetworkReachabilityChanged);
}

/*!
    Destroys an HTTP downloader.

    Removes the downloaded file if FileDownloader::isAutoRemoveDownloadedFile()
   returns \c true or FileDownloader::setAutoRemoveDownloadedFile() was called
   with \c true.
*/
...
/*!
    Returns \c true if the file is downloaded.
*/
bool KDUpdater::HttpDownloader::isDownloaded() const { return d->downloaded; }
...
/*!
    Clones the HTTP downloader and assigns it the parent \a parent. Returns the
   new HTTP downloader.
*/
KDUpdater::HttpDownloader *
KDUpdater::HttpDownloader::clone(QObject *parent) const {
  ...
  while (d->http->bytesAvailable()) {
    ...
    while (written < read) {
      ...
      if (numWritten < 0) {
        ...
        d->shutDown();
        setDownloadAborted(
            tr("Cannot download %1. Writing to file \"%2\" failed: %3")
  ...
  if (followRedirects() && redirectUrl.isValid()) {
    ...
    if (url.isValid() && QInstaller::lcServer().isDebugEnabled() &&
      ...
      if (fi.suffix() != QLatin1String("sha1")) {
        ...
        qCDebug(QInstaller::lcServer)
            << "Using host:" << hostName << "for" << url.fileName()
            << "\nIP:" << hostAddresses;
      }
    }
    httpReadyRead();
...
void KDUpdater::HttpDownloader::httpReadProgress(qint64 done, qint64 total) {
  if (d->http) {
    const QUrl redirectUrl =
        d->http->attribute(QNetworkRequest::RedirectionTargetAttribute).toUrl();
    if (followRedirects() && redirectUrl.isValid())
  ...
  if (isDownloadResumed())
    emit downloadProgress(
        calcProgress(done + totalBytesDownloadedBeforeResume(),
                     total + totalBytesDownloadedBeforeResume()));
  else
...
void KDUpdater::HttpDownloader::startDownload(const QUrl &url) {
  ...
  connect(d->http, &QNetworkReply::downloadProgress, this,
          &HttpDownloader::httpReadProgress);
  connect(d->http, &QNetworkReply::finished, this,
          &HttpDownloader::httpReqFinished);
  connect(d->http, &QNetworkReply::errorOccurred, this,
          &HttpDownloader::httpError);
  ...
  if (!d->destination->isOpen()) {
    ...
    const QString fileName = d->destination->fileName();
    d->shutDown();
    setDownloadAborted(tr("Cannot download %1. Cannot create file \"%2\": %3")
                           .arg(url.toString(), fileName, error));
  }
}
...
void KDUpdater::HttpDownloader::onAuthenticationRequired(
...
void KDUpdater::HttpDownloader::onNetworkReachabilityChanged(
...
void KDUpdater::HttpDownloader::onSslErrors(QNetworkReply *reply,
  ...
  if (arguments.contains(QLatin1String("--script")) ||
      arguments.contains(QLatin1String("Script")) || ignoreSslErrors()) {
    reply->ignoreSslErrors();
    return;
  }
  // TODO: Remove above code once we have a proper implementation for message
  // box handler supporting methods used in the following code, right now we
  // return here cause the message box is not scriptable.
  ...
  msgBox.setText(
      tr("There was an error during connection to: %1.").arg(url().toString()));
  msgBox.setInformativeText(
      QString::fromLatin1("<ul><li>%1</li><li>%2</li></ul>")
          .arg(tr("This could be "
                  "a problem with the server's configuration, or it could be "
                  "someone trying to impersonate the "
                  "server."),
               tr("If you have connected to this server successfully in the "
                  "past or trust this server, "
