...
TabController::Private::Private()
    : m_init(false), m_networkSettingsChanged(false), m_gui(nullptr),
      m_core(nullptr) {}

TabController::Private::~Private() { delete m_gui; }
...
void TabController::setGui(QInstaller::PackageManagerGui *gui) {
  d->m_gui = gui;
  connect(d->m_gui, &PackageManagerGui::gotRestarted, this,
          &TabController::restartWizard);
}
...
int TabController::init() {
  if (!d->m_init) {
    d->m_init = true;
    // this should called as early as possible, to handle error message boxes
    // for example
    if (!d->m_controlScript.isEmpty()) {
      d->m_gui->loadControlScript(d->m_controlScript);
      qCDebug(QInstaller::lcInstallerInstallLog)
          << "Using control script:" << d->m_controlScript;
    }

    connect(d->m_gui, &QWizard::currentIdChanged, this,
            &TabController::onCurrentIdChanged);
    connect(d->m_gui, &PackageManagerGui::settingsButtonClicked, this,
            &TabController::onSettingsButtonClicked);
    connect(d->m_gui, &PackageManagerGui::aboutApplicationClicked, this,
            &TabController::onAboutApplicationClicked);
  }

  IntroductionPage *page = qobject_cast<IntroductionPage *>(
      d->m_gui->page(PackageManagerCore::Introduction));
  if (page) {
...
void TabController::restartWizard() {
  ...
  if (d->m_networkSettingsChanged) {
    ...
    d->m_core->settings().setDefaultRepositories(
        d->m_settings.defaultRepositories());
    d->m_core->settings().setTemporaryRepositories(
        d->m_settings.temporaryRepositories(),
  ...
  // Make sure we are writing the .dat file with the list of uninstall
  // operations already now. Otherwise we will write at the end of the next
  // updater run, with a potentially empty component list (if no updates are
  // found).
  d->m_core->writeMaintenanceTool();
...
void TabController::onSettingsButtonClicked() {
  ...
  connect(&dialog, &SettingsDialog::networkSettingsChanged, this,
          &TabController::onNetworkSettingsChanged);
  connect(&dialog, &SettingsDialog::clearLocalCacheClicked, this,
          &TabController::onClearCacheClicked);
  dialog.exec();
...
void TabController::onClearCacheClicked() {
  ...
  // QtConcurrent::run([this]{ return analysa(); }); or QtConcurrent::run([this]
  // -> qlonglong { return analysa(); });

  QFuture<bool> future = QtConcurrent::run(
      [&]() { return d->m_core->clearLocalCache(&errorMessage); });
  ...
  msgBox.setText(success ? tr("Cache cleared successfully!") : errorMessage);
...
void TabController::onCurrentIdChanged(int newId) {
  if (d->m_gui) {
    if (PackageManagerPage *page =
            qobject_cast<PackageManagerPage *>(d->m_gui->page(newId)))
      d->m_gui->showSettingsButton(page->settingsButtonRequested());
  }
}

void TabController::onNetworkSettingsChanged(
    const QInstaller::Settings &settings) {
